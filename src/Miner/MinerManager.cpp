...
namespace Miner {
                ...
                case MinerEventType::BLOCK_MINED: {
                    ...
                    if (submitBlock (m_minedBlock)) {
                        ...
                        if (m_config.blocksLimit != 0 && ++blocksMined == m_config.blocksLimit) {
                            std::cout
                                << InformationMsg ("Mined requested amount of blocks (")
    ...
    bool MinerManager::submitBlock(const CryptoNote::BlockTemplate &minedBlock)
    {
        ...
        if (!res || res->status == 200) {
            ...
            std::cout
                << WarningMsg ("Failed to submit block, possibly daemon offline or syncing?\n");
            return false;
        }
    }
    ...
    CryptoNote::BlockMiningParameters MinerManager::requestMiningParameters()
    {
        while (true) {
            ...
            if (!res) {
                std::cout
                    << WarningMsg ("Failed to get block template - Is your daemon open?\n");
            ...
            if (res->status != 200) {
                ...
                stream
                    << "Failed to get block template - received unexpected http "
                    << "code from server: "
                    << res->status
                    << std::endl;
            ...
            try {
                ...
                if (status != "OK") {
                    ...
                    stream
                        << "Failed to get block template from daemon. Response: "
                        << status
                        << std::endl;
                ...
                if (!fromBinaryArray (params.blockTemplate, blob)) {
                    std::cout
                        << WarningMsg ("Couldn't parse block template from daemon.")
                        << std::endl;
                ...
                stream
                    << "Failed to parse block template from daemon. Received data:\n"
                    << res->body
                    << "\nParse error: "
                    << e.what ()
                    << std::endl;
