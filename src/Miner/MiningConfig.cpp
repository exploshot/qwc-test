...
namespace CryptoNote {
    ...
    void MiningConfig::parse(int argc, char **argv)
    {
        ...
        options.add_options ("Daemon")
                   ("daemon-address",
                    "The daemon [host:port] combination to use for node operations. This option overrides --daemon-host and --daemon-rpc-port",
        ...
        options.add_options ("Mining")
                   ("address",
                    ...
                    cxxopts::value<std::string> (miningAddress),
                    "<address>")
        ...
        try {
            ...
            std::cout
                << WarningMsg ("Error: Unable to parse command line argument options: ")
                << WarningMsg (e.what ())
                << "\n\n";
            std::cout
                << options.help ({})
         ...
         */
            ...
            std::cout
                << InformationMsg (getProjectCLIHeader ())
                << std::endl;
            exit (0);
        }
        ...
        while (error != SUCCESS) {
            ...
            if (!miningAddress.empty ()) {
                std::cout
                    << WarningMsg ("Address is not valid: ")
                    << WarningMsg (error)
                    << std::endl;
            }

            std::cout
                << InformationMsg ("What address do you want to mine to?: ");
