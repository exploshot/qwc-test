...
void blake256Update(state *S, const uint8_t *data, uint64_t datalen)
{
    int left = S->buflen
        >> 3;
    int fill = 64 - left;
...
void blake256Final_h(state *S, uint8_t *digest, uint8_t pa, uint8_t pb)
{
    uint8_t msglen[8];
...
{
    blake256Final_h (S, digest, 0x81, 0x01);
}

// inlen = number of bytes
void blake256Hash(uint8_t *out, const uint8_t *in, uint64_t inlen)
{
    state S;
    blake256Init (&S);
    blake256Update (&S, in, inlen * 8);
    blake256Final (&S, out);
}
