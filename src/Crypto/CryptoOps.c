...
static void geMAdd(geP1P1 *, const geP3 *, const gePrecomp *);
static void geMSub(geP1P1 *, const geP3 *, const gePrecomp *);
static void geP20(geP2 *);
static void geP3Dbl(geP1P1 *, const geP3 *);
...
void geDsmPrecomp(geDsmp r, const geP3 *s) 
{
    ...
    geP3 s2, u;
    geP3ToCached(&r[0], s);
    geP3Dbl(&t, s); geP1P1ToP3(&s2, &t);
    geAdd(&t, &s2, &r[0]); geP1P1ToP3(&u, &t); geP3ToCached(&r[1], &u);
    geAdd(&t, &s2, &r[1]); geP1P1ToP3(&u, &t); geP3ToCached(&r[2], &u);
    geAdd(&t, &s2, &r[2]); geP1P1ToP3(&u, &t); geP3ToCached(&r[3], &u);
    geAdd(&t, &s2, &r[3]); geP1P1ToP3(&u, &t); geP3ToCached(&r[4], &u);
    geAdd(&t, &s2, &r[4]); geP1P1ToP3(&u, &t); geP3ToCached(&r[5], &u);
    geAdd(&t, &s2, &r[5]); geP1P1ToP3(&u, &t); geP3ToCached(&r[6], &u);
    geAdd(&t, &s2, &r[6]); geP1P1ToP3(&u, &t); geP3ToCached(&r[7], &u);
}
                                   ...
                                   const unsigned char *b) 
{
    ...
    for (; i >= 0; --i) {
        ...
        if (bslide[i] > 0) {
            geP1P1ToP3(&u, &t);
            geMAdd(&t, &u, &ge_Bi[bslide[i]/2]);
        } else if (bslide[i] < 0) {
            geP1P1ToP3(&u, &t);
            geMSub(&t, &u, &ge_Bi[(-bslide[i])/2]);
        }
...
static void geCachedCmov(geCached *t, 
                         const geCached *u, 
                         unsigned char b) 
{
    feCmov(t->YplusX, u->YplusX, b);
...
int geCheckSubgroupPrecompVartime(const geDsmp p) 
{
    geP3 s;
    geP1P1 t;
    geP2 u;
...
void geFromFeFromBytesVartime(geP2 *r, const unsigned char *s) 
{
    ...
    if (feIsNonZero(y)) {
        ...
        if (feIsNonZero(y)) {
            ...
            feMul(r->X, r->X, fe_fffb1);
        }
    } else {
        feMul(r->X, r->X, fe_fffb2);
    }
    ...
    if (feIsNonZero(y)) {
        ...
        feMul(r->X, r->X, fe_fffb3);
    } else {
        feMul(r->X, r->X, fe_fffb4);
    }
    /* r->X = sqrt(A * (A + 2) * w / x) */
    /* z = -A */
    sign = 1;
