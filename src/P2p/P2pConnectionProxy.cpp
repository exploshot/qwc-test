...
namespace CryptoNote {
    ...
    bool P2pConnectionProxy::processIncomingHandshake()
    {
        ...
        if (cmd.command == COMMAND_PING::ID) {
            ...
            m_context.writeMessage (makeReply (COMMAND_PING::ID,
                                               LevinProtocol::encode (resp),
    ...
    void P2pConnectionProxy::writeHandshake(const P2pMessage &message)
    {
        ...
        if (m_context.isIncoming ()) {
            ...
            m_context.writeMessage (makeReply (COMMAND_HANDSHAKE::ID,
    ...
    void P2pConnectionProxy::handleTimedSync(const LevinProtocol::Command &cmd)
    {
        if (cmd.isResponse) {
            ...
            m_context.writeMessage (makeReply (COMMAND_TIMED_SYNC::ID,
                                               LevinProtocol::encode (res),
                                               LEVIN_PROTOCOL_RETCODE_SUCCESS));
        }
    }
