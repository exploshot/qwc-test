...
std::shared_ptr<WalletInfo> mnemonicImportWallet(CryptoNote::WalletGreen &wallet)
{
    ...
    while (true) {
        ...
        auto[error, privateSpendKey] = Mnemonics::MnemonicToPrivateKey (mnemonicPhrase, defaultLan);
...
std::shared_ptr<WalletInfo> generateWallet(CryptoNote::WalletGreen &wallet)
{
    ...
    Crypto::CryptoOps::generateViewFromSpend (spendKey.secretKey,
                                              privateViewKey);
    ...
    return std::make_shared<WalletInfo> (walletFileName,
                                         walletPass,
                                         walletAddress,
                                         false,
                                         wallet);
    ...
    while (true) {
        ...
        try {
            ...
            if (errorMsg == walletSuccessBadPwdMsg ||
                ...
                std::cout
                    << WarningMsg ("Could not open wallet! It is already "
                ...
                std::cout
                    << WarningMsg ("Could not open wallet file! It "
...
Crypto::SecretKey getPrivateKey(std::string msg)
{
    ...
    while (true) {
        ...
        if (privateKeyString.length () != privateKeyLen) {
            ...
            std::cout
                << WarningMsg ("Invalid private key, failed to parse! "
...
{
    std::cout
        << InformationMsg ("Please remember that when using a view wallet "
