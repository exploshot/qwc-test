...
std::shared_ptr<WalletInfo> createViewWallet(CryptoNote::WalletGreen &wallet)
{
    ...
    wallet.createViewWallet(
        walletFileName, walletPass, address,
...
std::shared_ptr<WalletInfo> mnemonicImportWallet(CryptoNote::WalletGreen &wallet)
{
    ...
    while (true) {
        std::cout
            << "Enter your mnemonic phrase (25 words): ";
                                           ...
                                           Crypto::SecretKey privateViewKey)
{
    ...
    std::string walletAddress = wallet.createAddress(privateSpendKey, scanHeight, false);
...
std::shared_ptr<WalletInfo> generateWallet(CryptoNote::WalletGreen &wallet)
{
    ...
    std::string walletAddress = wallet.createAddress(spendKey.secretKey, 0, true);

    promptSaveKeys(wallet);

    std::cout
        << WarningMsg("If you lose these your wallet cannot be recreated!")
        << std::endl
        << std::endl;
    ...
    while (true) {
        ...
        try {
            ...
            if (errorMsg == walletSuccessBadPwdMsg ||
                ...
                std::cout
                    << WarningMsg(
                        "Could not open wallet file! It "
                        "doesn't appear to be a valid wallet!"
                    )
                    << std::endl
                    << WarningMsg(
                        "Ensure you are opening a wallet "
                        "file, and the file has not gotten "
                        "corrupted."
                    )
                    << std::endl
                    << WarningMsg(
                        "Try reimporting via keys, and always "
                ...
                std::cout
                    ...
                    << std::endl;
                std::cout
                    << "Please report this error message and what "
                    << "you did to cause it."
                    << std::endl
                    << std::endl;
...
Crypto::SecretKey getPrivateKey(std::string msg)
{
    ...
    while (true) {
        ...
        if (privateKeyString.length() != privateKeyLen) {
            std::cout
                << WarningMsg(
                    "Invalid private key, should be 64 "
            ...
            privateKeyString, &privateKeyHash,
            sizeof(privateKeyHash), size
        )
            || size != sizeof(privateKeyHash)) {
            std::cout
                << WarningMsg(
                    "Invalid private key, failed to parse! "
        ...
        /* Just used for verification purposes before we pass it to
           walletgreen */
        if (!Crypto::secretKeyToPublicKey(privateKey, publicKey)) {
            std::cout
                << "Invalid private key, failed to parse! Ensure "
...
std::string getExistingWalletFileName(Config &config)
{
    ...
    while (true) {
        ...
        if (config.walletGiven && initial) {
            ...
            std::cout
                << "What is the name of the wallet you want to open?: ";
...
std::string getNewWalletFileName()
{
    ...
    while (true) {
        std::cout
            << "What would you like to call your new wallet?: ";
        std::getline(std::cin, walletName);
...
std::string getWalletPassword(bool verifyPwd)
{
    ...
    pwdContainer.readPassword(verifyPwd);
...
void viewWalletMsg()
{
    std::cout
        ...
        << std::endl
        << "This means if you received 100 TRTL and then "
...
void connectingMsg()
{
    std::cout
        << std::endl
        << "Making initial contact with QDaemon."
        << std::endl
