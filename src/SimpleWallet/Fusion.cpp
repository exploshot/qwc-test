...
size_t makeFusionTransaction(CryptoNote::WalletGreen &wallet, uint64_t threshold)
{
    ...
    try {
        ...
        std::cout
            << WarningMsg ("Failed to send fusion transaction: ")
            << WarningMsg (e.what ())
            << std::endl;
...
{
    std::cout
        << "Attempting to optimize your wallet to allow you to send "
        << "large amounts at once. "
        << std::endl
        << "You can run this command as many times as you like."
        << std::endl
        << "You will be informed when your wallet is fully "
        << "optimized."
        << std::endl
        << WarningMsg ("This may take a long time!")
        << std::endl;

    if (!confirm ("Do you want to proceed?")) {
        std::cout
            << WarningMsg ("Cancelling optimization.")
            << std::endl;
        return;
    }
    ...
    if (!optimize (wallet, wallet.getActualBalance ())) {
        std::cout
            << SuccessMsg ("Wallet fully optimized!")
            << std::endl;
    } else {
        std::cout
            << SuccessMsg ("Optimization completed!")
            << std::endl
...
{
    std::cout
        << "Attempting to optimize your wallet to allow you to "
    ...
    for (int i = 1;; i++) {
        std::cout
            << InformationMsg ("Running optimization round " +
                               std::to_string (i) +
                               "...")
            << std::endl;
    ...
    std::cout
        << SuccessMsg ("Full optimization completed!")
        << std::endl;
}
...
bool optimize(CryptoNote::WalletGreen &wallet, uint64_t threshold)
{
    ...
    while (true) {
        ...
        if (tmpFusionTxID == CryptoNote::WALLET_INVALID_TRANSACTION_ID) {
            ...
            if (fusionTransactionHashes.size () == 1) {
                std::cout
                    << SuccessMsg ("Created 1 fusion transaction!")
                    << std::endl;
    ...
    if (fusionTransactionHashes.size () == 1) {
        std::cout
            << SuccessMsg ("1 fusion transaction has been send, waiting "
                           "for balance to return and unlock")
            << std::endl
            << std::endl;
    } else {
        std::cout
            << SuccessMsg (std::to_string (fusionTransactionHashes.size ()) +
    ...
    /*!
     * short sleep to ensure its in the transaction pool when we poll it
     */
    ...
    while (true) {
        ...
        for (auto tx : unconfirmedTransactions) {
            unconfirmedTxHashes.push_back (tx.transaction.hash);
        }
        ...
        if (!fusionCompleted) {
            std::cout
                << WarningMsg ("Balance still locked, " +
                               std::to_string (fusionTransactionHashes.size ()));

            if (fusionTransactionHashes.size () == 1) {
                std::cout
                    << WarningMsg (" fusion transaction still to be confirmed.");
            } else {
                std::cout
            ...
            std::cout
                << SuccessMsg ("All fusion transactions confirmed!")
                << std::endl;
            break;
        }
    }
...
{
    std::cout
        << WarningMsg ("Your transaction is too large to be accepted by "
                       "the network!")
        << std::endl
    ...
    while (wallet.getActualBalance () < p.destinations[0].amount + p.fee) {
        ...
        if ((std::chrono::system_clock::now () - startTime) >
            ...
            std::cout
                << WarningMsg ("Fusion transactions have "
        ...
        std::cout
            << WarningMsg ("Optimization completed, but balance "
