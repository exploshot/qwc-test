...
bool confirmTransaction(CryptoNote::TransactionParameters t, std::shared_ptr<WalletInfo> walletInfo)
{
    std::cout
        << std::endl
        << InformationMsg ("Confirm Transaction?")
        << std::endl;
    ...
    std::cout
        ...
        << std::endl
        << "FROM: "
        << SuccessMsg (walletInfo->walletFileName)
        << std::endl
        << "TO: "
        << SuccessMsg (t.destinations[0].address)
        << std::endl
        << std::endl;
                              ...
                              std::vector<CryptoNote::TransactionParameters> transfers)
{
    ...
    std::cout
        << "Your transaction has been split up into "
        << numTxs
    ...
    for (auto tx : transfers) {
        while (true) {
            std::cout
                ...
                << " of "
                << InformationMsg (std::to_string (numTxs))
                << std::endl;
            ...
            if (neededBalance < wallet.getActualBalance ()) {
                ...
                std::cout
                    << SuccessMsg ("Transaction has been sent!")
                    << std::endl
                    << SuccessMsg ("Hash: "
            ...
            std::cout
                << "Waiting for balance to unlock to send next "
                << "transaction."
                << std::endl
    ...
    std::cout
        << SuccessMsg ("All transactions sent!")
        << std::endl;
}
...
void splitTx(CryptoNote::WalletGreen &wallet, CryptoNote::TransactionParameters p)
{
    ...
    for (int numTxMultiplier = 2;; numTxMultiplier++) {
         ...
         * QWC was requested and we split it into 4 transactions each one will
         * have a fee of 5 QWC. If the fee per transaction is less than the
...
{
    std::cout
        << InformationMsg ("Note: You can type cancel at any time to "
    ...
    if (balance < amount) {
        std::cout
            << WarningMsg ("You don't have enough funds to cover this "
                           "transaction!")
            << std::endl
                ...
                uint32_t height)
{
    ...
    while (true) {
        try {
            ...
            if (retried) {
                std::cout
                    << WarningMsg ("Failed to send transaction!")
                    << std::endl
                    << "Error message: "
                    << e.what ()
                    << std::endl;
            ...
            switch (e.code ().value ()) {
                ...
                case NodeErrors::CryptoNote::NodeError::INTERNAL_NODE_ERROR: {
                    ...
                    /*!
                     * Mixin 0 not allowed after MIXIN_LIMITS_V2Mixin 0 not allowed after MIXIN_LIMITS_V2
                     */
                    if (height < CryptoNote::parameters::MIXIN_LIMITS_V2_HEIGHT) {
                        std::cout
                            << "Alternatively, you can sent the mixin "
                            << "count to 0."
                            << std::endl;
                ...
                case NodeErrors::CryptoNote::NodeError::NETWORK_ERROR:
                case NodeErrors::CryptoNote::NodeError::CONNECT_ERROR: {
                    std::cout
                        << WarningMsg ("Couldn't connect to the network "
                                       "to send the transaction!")
                        << std::endl
                        << "Ensure QDaemon or the remote node you "
...
Maybe<std::string> getPaymentID()
{
    while (true) {
        ...
        if (!CryptoNote::createTxExtraWithPaymentId (paymentID, extra)) {
            std::cout
                << WarningMsg ("Failed to parse! Payment ID's are 64 "
...
Maybe<uint64_t> getTransferAmount()
{
    while (true) {
        ...
        std::cout
            << std::endl
            << InformationMsg ("How much " + WalletConfig::ticker + " do you want to send?: ");
...
Maybe<std::string> getDestinationAddress()
{
    while (true) {
        ...
        std::cout
            << InformationMsg ("What address do you want to "
                               "transfer to?: ");
...
bool parseFee(std::string feeString)
{
    ...
    if (!parseAmount (feeString, fee)) {
        std::cout
            << WarningMsg ("Failed to parse fee! Ensure you entered the "
                           "value correctly.")
            << std::endl
        ...
        std::cout
            << WarningMsg ("Fee must be at least " +
                           formatAmount (CryptoNote::parameters::MINIMUM_FEE) +
                           " " + WalletConfig::ticker + "!")
            << std::endl;
        return false;
...
bool parseAddress(std::string address)
{
    ...
    bool valid = Utilities::parseAccountAddressString (prefix, addr, address);
    ...
    Crypto::generateKeys (viewKey.publicKey, viewKey.secretKey);

    CryptoNote::AccountPublicAddress expectedAddr
        {spendKey.publicKey, viewKey.publicKey};

    size_t expectedLen = Utilities::getAccountAddressAsStr (expectedPrefix,
                                                            expectedAddr).length ();

    if (address.length () != expectedLen) {
        std::cout
            << WarningMsg ("Address is wrong length!")
            << std::endl
            << "It should be "
            << expectedLen
            << " characters long, but it is "
            << address.length ()
            << " characters long!"
            << std::endl
            << std::endl;
         ...
         * with QWC, aslong as the prefix is the QWC prefix. This keeps it
         * working on testnets with different prefixes.
         */
    else if (expectedPrefix == CryptoNote::parameters::CRYPTONOTE_PUBLIC_ADDRESS_BASE58_PREFIX
             && address.substr (0, WalletConfig::ticker.length ()) != WalletConfig::ticker) {
        std::cout
            << WarningMsg ("Invalid address! It should start with " + WalletConfig::ticker + "!")
            << std::endl
            << std::endl;
        return false;
    }
        ...
        std::cout
            << WarningMsg ("Failed to parse address, address is not a "
                           "valid " + WalletConfig::ticker + " address!")
            << std::endl
            << std::endl;
        return false;
    }
...
bool parseAmount(std::string amountString)
{
    ...
    if (!parseAmount (amountString, amount)) {
        std::cout
            << WarningMsg ("Failed to parse amount! Ensure you entered "
                           "the value correctly.")
            << std::endl
