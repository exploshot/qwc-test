                           ...
                           bool fetchTimestamp)
{
    ...
    if (paymentID != "") {
        std::cout
            << WarningMsg ("Payment ID: " + paymentID)
            << std::endl;
    }
    ...
    if (fetchTimestamp) {
        ...
        /*!
         * Couldn't get timestamp, maybe old node or QDaemon closed
         */
        if (blockTime != "") {
            std::cout
                << WarningMsg ("Timestamp: " + blockTime)
                << std::endl;
        }
    }
                           ...
                           bool fetchTimestamp)
{
    ...
    if (paymentID != "") {
        std::cout
            << SuccessMsg ("Payment ID: " + paymentID)
            << std::endl;
    }
    ...
    if (fetchTimestamp) {
        ...
        if (blockTime != "") {
            std::cout
                << SuccessMsg ("Timestamp: " + blockTime)
                << std::endl;
        }
    }
...
void listTransfers(bool incoming,
                   bool outgoing,
...
void saveCSV(CryptoNote::WalletGreen &wallet, CryptoNote::INode &node)
{
    ...
    if (!myfile) {
        std::cout
            << WarningMsg ("Couldn't open transactions.csv file for saving!")
            << std::endl;
        std::cout
            << WarningMsg ("Ensure it is not open in any other application.")
            << std::endl;
        return;
    }
    std::cout
        << InformationMsg ("Saving CSV file...")
        << std::endl;
     ...
     */
    myfile
        << "Block date/time,Block Height,Hash,Amount,Currency,In/Out\n";
    ...
    for (size_t i = 0; i < numTransactions; i++) {
        ...
        if (t.totalAmount != 0) {
            ...
            if (t.totalAmount < 0) {
                /*!
                 * Put QWC in separate field, makes output more usable in spreadsheet
                 */
                std::string splitAmtQWC = formatAmount (-t.totalAmount);
                boost::replace_all (splitAmtQWC, " ", ",");
                myfile
                    << "-"
                    << splitAmtQWC
                    << ",OUT\n";
    ...
    myfile.close ();
    std::cout
        << SuccessMsg ("CSV file saved successfully.")
        << std::endl;
}
                ...
                std::shared_ptr<WalletInfo> &walletInfo)
{
    ...
    if (localHeight != remoteHeight) {
        std::cout
            << "Your Node isn't fully synced yet!"
            << std::endl
    ...
    if (walletHeight == 1 && transactionCount != 0) {
        std::cout
            << "Upgrading your wallet from an older version of the "
            << "software..."
            << std::endl
    ...
    if (walletHeight == 1) {
        std::cout
            << "Scanning through the blockchain to find transactions "
            << "that belong to you."
            << std::endl
        ...
        std::cout
            << "Scanning through the blockchain to find any new "
    ...
    while (walletHeight < localHeight) {
        ...
        std::cout
            ...
            << " of "
            << InformationMsg (std::to_string (localHeight))
            << std::endl;
        ...
        if (tmpWalletHeight == walletHeight) {
            ...
            if (stuckCounter > 20) {
                std::string warning =
                    "Syncing may be stuck. Try restarting QDaemon.\n"
                    "If this persists, visit "
                    "https://chat.qwertycoin.org for support.";
                std::cout
                    << WarningMsg (warning)
                    << std::endl;
    ...
    std::cout
        << std::endl
        << SuccessMsg ("Finished scanning blockchain!")
        << std::endl
        << std::endl;
