...
ApiDispatcher::ApiDispatcher(const uint16_t bindPort,
      ...
      m_rpcPassword (rpcPassword)
{
    ...
    m_server.Post ("/wallet/open", router (&ApiDispatcher::openWallet, WalletMustBeClosed, viewWalletsAllowed))
                 ...
                 */
            .Post ("/wallet/import/key",
                   router (&ApiDispatcher::keyImportWallet, WalletMustBeClosed, viewWalletsAllowed))

                /*!
                 * Import wallet with seed
                 */
            .Post ("/wallet/import/seed",
                   router (&ApiDispatcher::seedImportWallet, WalletMustBeClosed, viewWalletsAllowed))

                /*!
                 * Import view wallet
                 */
            .Post ("/wallet/import/view",
                   router (&ApiDispatcher::importViewWallet, WalletMustBeClosed, viewWalletsAllowed))
            ...
            .Post ("/addresses/import", router (&ApiDispatcher::importAddress, WalletMustBeOpen, viewWalletsBanned))
                 ...
                 */
            .Post ("/addresses/import/view",
                   router (&ApiDispatcher::importViewAddress, WalletMustBeOpen, viewWalletsAllowed))
            ...
            .Get ("/addresses/primary",
                  router (&ApiDispatcher::getPrimaryAddress, WalletMustBeOpen, viewWalletsAllowed))
            ...
            .Get ("/transactions/address/" + ApiConstants::addressRegex + "/\\d+", router (
                ...
                /*!
                 * Get the transactions starting at the given block,
                 * and ending at the given block, belonging to the given address
                 */
...
void ApiDispatcher::middleware(const Request &req,
                               Response &res,
                               const WalletState walletState,
                               const bool viewWalletPermitted,
                                                  ...
                                                  const nlohmann::json &body)> handler)
{
    ...
    try {
        ...
        std::cout
            << "Caught JSON exception, likely missing required "
               "json parameter: "
            << e.what ()
            << std::endl;
        res.status = 400;
...
bool ApiDispatcher::checkAuthenticated(const Request &req, Response &res) const
{
    if (!req.has_header ("X-API-KEY")) {
        std::cout
            << "Rejecting unauthorized request: X-API-KEY header is missing.\n";
    ...
    std::cout
        << "Rejecting unauthorized request: X-API-KEY is incorrect.\n"
           "Expected: "
        << m_rpcPassword
        << "\nActual: "
        << apiKey
...
std::tuple <Error, uint16_t> ApiDispatcher::openWallet(const Request &req,
                                                       Response &res,
                                                       const nlohmann::json &body)
{
    ...
    std::tie (error, m_walletBackend) = WalletBackend::openWallet (filename,
                                                                   password,
                                                                   daemonHost,
                                                                   daemonPort,
                                                                   daemonSSL,
...
std::tuple <Error, uint16_t> ApiDispatcher::keyImportWallet(const Request &req,
                                                            Response &res,
                                                            const nlohmann::json &body)
{
    std::scoped_lock lock (m_mutex);
...
std::tuple <Error, uint16_t> ApiDispatcher::seedImportWallet(const Request &req,
                                                             Response &res,
                                                             const nlohmann::json &body)
{
    std::scoped_lock lock (m_mutex);
    ...
    std::tie (error, m_walletBackend) = WalletBackend::importWalletFromSeed (mnemonicSeed,
                                                                             filename,
                                                                             password,
                                                                             scanHeight,
                                                                             daemonHost,
...
std::tuple <Error, uint16_t> ApiDispatcher::importViewWallet(const Request &req,
                                                             Response &res,
                                                             const nlohmann::json &body)
{
    std::scoped_lock lock (m_mutex);
    ...
    std::tie (error, m_walletBackend) = WalletBackend::importViewWallet (privateViewKey,
                                                                         address,
                                                                         filename,
                                                                         password,
                                                                         scanHeight,
...
std::tuple <Error, uint16_t> ApiDispatcher::createWallet(const Request &req,
                                                         Response &res,
                                                         const nlohmann::json &body)
{
    std::scoped_lock lock (m_mutex);
    ...
    std::tie (error, m_walletBackend) = WalletBackend::createWallet (filename,
                                                                     password,
                                                                     daemonHost,
                                                                     daemonPort,
                                                                     daemonSSL,
...
std::tuple <Error, uint16_t> ApiDispatcher::createAddress(const Request &req,
                                                          Response &res,
                                                          const nlohmann::json &body)
{
...
std::tuple <Error, uint16_t> ApiDispatcher::importAddress(const Request &req,
                                                          Response &res,
                                                          const nlohmann::json &body)
{
    uint64_t scanHeight = 0;
...
std::tuple <Error, uint16_t> ApiDispatcher::importViewAddress(const Request &req,
                                                              Response &res,
                                                              const nlohmann::json &body)
{
    uint64_t scanHeight = 0;
...
std::tuple <Error, uint16_t> ApiDispatcher::validateAddress(const Request &req,
                                                            Response &res,
                                                            const nlohmann::json &body)
{
    const std::string address = getJsonValue<std::string> (body, "address");
...
std::tuple <Error, uint16_t> ApiDispatcher::sendBasicTransaction(const Request &req,
                                                                 Response &res,
                                                                 const nlohmann::json &body)
{
...
std::tuple <Error, uint16_t> ApiDispatcher::sendAdvancedTransaction(const Request &req,
                                                                    Response &res,
                                                                    const nlohmann::json &body)
{
    ...
    if (body.find ("sourceAddresses") != body.end ()) {
        subWalletsToTakeFrom = getJsonValue
                               < std::vector
                               < std::string
                                   >> (body, "sourceAddresses");
    }
...
std::tuple <Error, uint16_t> ApiDispatcher::sendBasicFusionTransaction(const Request &req,
                                                                       Response &res,
                                                                       const nlohmann::json &body)
{
...
std::tuple <Error, uint16_t> ApiDispatcher::sendAdvancedFusionTransaction(const Request &req,
                                                                          Response &res,
                                                                          const nlohmann::json &body)
{
 ...
 * DELETE REQUESTS
 */
std::tuple <Error, uint16_t> ApiDispatcher::closeWallet(const Request &req,
                                                        Response &res,
                                                        const nlohmann::json &body)
{
    std::scoped_lock lock (m_mutex);
...
std::tuple <Error, uint16_t> ApiDispatcher::deleteAddress(const Request &req,
                                                          Response &res,
                                                          const nlohmann::json &body)
{
    /*!
 ...
 * PUT REQUESTS
 */
std::tuple <Error, uint16_t> ApiDispatcher::saveWallet(const Request &req,
                                                       Response &res,
...
std::tuple <Error, uint16_t> ApiDispatcher::resetWallet(const Request &req,
                                                        Response &res,
                                                        const nlohmann::json &body)
{
    std::scoped_lock lock (m_mutex);
...
std::tuple <Error, uint16_t> ApiDispatcher::setNodeInfo(const Request &req,
                                                        Response &res,
                                                        const nlohmann::json &body)
{
    std::scoped_lock lock (m_mutex);
...
std::tuple <Error, uint16_t> ApiDispatcher::getPrivateViewKey(const Request &req,
                                                              Response &res,
                                                              const nlohmann::json &body) const
{
    nlohmann::json j{
 ...
 */
std::tuple <Error, uint16_t> ApiDispatcher::getSpendKeys(const Request &req,
                                                         Response &res,
                                                         const nlohmann::json &body) const
{
    /*!
 ...
 */
std::tuple <Error, uint16_t> ApiDispatcher::getMnemonicSeed(const Request &req,
                                                            Response &res,
                                                            const nlohmann::json &body) const
{
    /*!
...
std::tuple <Error, uint16_t> ApiDispatcher::getStatus(const Request &req,
                                                      Response &res,
                                                      const nlohmann::json &body) const
{
...
std::tuple <Error, uint16_t> ApiDispatcher::getAddresses(const Request &req,
                                                         Response &res,
                                                         const nlohmann::json &body) const
{
    nlohmann::json j{
...
std::tuple <Error, uint16_t> ApiDispatcher::getPrimaryAddress(const Request &req,
                                                              Response &res,
                                                              const nlohmann::json &body) const
{
    nlohmann::json j{
...
std::tuple <Error, uint16_t> ApiDispatcher::createIntegratedAddress(const Request &req,
                                                                    Response &res,
                                                                    const nlohmann::json &body) const
{
...
std::tuple <Error, uint16_t> ApiDispatcher::getTransactions(const Request &req,
                                                            Response &res,
                                                            const nlohmann::json &body) const
{
    nlohmann::json j{
...
std::tuple <Error, uint16_t> ApiDispatcher::getUnconfirmedTransactions(const Request &req,
                                                                       Response &res,
                                                                       const nlohmann::json &body) const
{
    nlohmann::json j{
...
std::tuple <Error, uint16_t> ApiDispatcher::getUnconfirmedTransactionsForAddress(const Request &req,
                                                                                 Response &res,
                                                                                 const nlohmann::json &body) const
{
                                                                      ...
                                                                      const nlohmann::json &body) const
{
    ...
    try {
        ...
        std::cout
            << "Height parameter is too large or too small!"
            << std::endl;
        return {SUCCESS, 400};
    } catch (const std::invalid_argument &e) {
        std::cout
            << "Failed to parse parameter as height: "
            << e.what ()
            << std::endl;
        return {SUCCESS, 400};
    }
...
std::tuple <Error, uint16_t> ApiDispatcher::getTransactionsFromHeightToHeightWithAddress(const httplib::Request &req,
                                                                                         httplib::Response &res,
                                                                                         const nlohmann::json &body) const
{
    std::string stripped = req.path.substr (std::string ("/transactions/address/").size ());
...
std::tuple <Error, uint16_t> ApiDispatcher::getTransactionDetails(const httplib::Request &req,
                                                                  httplib::Response &res,
                                                                  const nlohmann::json &body) const
{
...
std::tuple <Error, uint16_t> ApiDispatcher::getBalance(const httplib::Request &req,
                                                       httplib::Response &res,
                                                       const nlohmann::json &body) const
{
...
std::tuple <Error, uint16_t> ApiDispatcher::getBalanceForAddress(const httplib::Request &req,
                                                                 httplib::Response &res,
                                                                 const nlohmann::json &body) const
{
...
std::tuple <Error, uint16_t> ApiDispatcher::getBalances(const httplib::Request &req,
                                                        httplib::Response &res,
                                                        const nlohmann::json &body) const
{
...
std::tuple <Error, uint16_t> ApiDispatcher::getTxPrivateKey(const httplib::Request &req,
                                                            httplib::Response &res,
                                                            const nlohmann::json &body) const
{
 ...
 * OPTIONS REQUESTS
 */
void ApiDispatcher::handleOptions(const Request &req,
                                  Response &res) const
{
    std::cout
        << "Incoming "
...
bool ApiDispatcher::assertIsNotViewWallet() const
{
    if (m_walletBackend->isViewWallet ()) {
        std::cout
            << "Client requested to perform an operation which requires "
...
bool ApiDispatcher::assertWalletClosed() const
{
    if (m_walletBackend != nullptr) {
        std::cout
            << "Client requested to open a wallet, whilst one is already open"
            << std::endl;
        return false;
    }
...
bool ApiDispatcher::assertWalletOpen() const
{
    if (m_walletBackend == nullptr) {
        std::cout
            << "Client requested to modify a wallet, whilst no wallet is open"
            << std::endl;
        return false;
    }
