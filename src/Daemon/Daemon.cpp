...
int main(int argc, char *argv[])
{
    ...
    /*!
     * Do we want to generate the Genesis Tx?
     */
    if (config.printGenesisTx) {
    ...
    try {
        ...
        if (use_checkpoints) {
            logger (INFO)
                << "Loading Checkpoints for faster initial sync...";
        ...
        if (!Tools::createDirectoriesIfNecessary (dbConfig.getDataDir ())) {
        ...
        */
        ...
        if (!config.banImportFile.empty ()) {
            if (!boost::filesystem::exists (config.banImportFile)) {
                logger (WARNING)
                    << "Unable to import banned ips, file not found: "
                    << config.banImportFile;
            }
        ...
        logger (INFO)
            << "Starting core rpc server on address "
            << config.rpcInterface
            << ":"
            << config.rpcPort;
