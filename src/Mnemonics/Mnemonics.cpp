...
namespace Language {
    ...
    const static LanguageMap cLanguageMap = {
        ...
        {WordLists::German::cName, Lazy<std::shared_ptr<Base>> ([]()
                                                                {
                                                                    return std::make_shared<WordLists::German> ();
                                                                })},
        {WordLists::Italian::cName, Lazy<std::shared_ptr<Base>> ([]()
                                                                 {
                                                                     return std::make_shared<WordLists::Italian> ();
                                                                 })},

        {WordLists::Ukrainian::cName, Lazy<std::shared_ptr<Base>> ([]()
                                                                   {
                                                                       return std::make_shared<WordLists::Ukrainian> ();
                                                                   })},
        {WordLists::Polish::cName, Lazy<std::shared_ptr<Base>> ([]()
                                                                {
                                                                    return std::make_shared<WordLists::Polish> ();
                                                                })},

        {WordLists::Spanish::cName, Lazy<std::shared_ptr<Base>> ([]()
                                                                 {
                                                                     return std::make_shared<WordLists::Spanish> ();
                                                                 })},

        {WordLists::Portugese::cName, Lazy<std::shared_ptr<Base>> ([]()
                                                                   {
                                                                       return std::make_shared<WordLists::Portugese> ();
                                                                   })},
        {WordLists::Dutch::cName, Lazy<std::shared_ptr<Base>> ([]()
                                                               {
                                                                   return std::make_shared<WordLists::Dutch> ();
                                                               })},
        {WordLists::Japanese::cName, Lazy<std::shared_ptr<Base>> ([]()
                                                                  {
                                                                      return std::make_shared<WordLists::Japanese> ();
                                                                  })},

        {WordLists::Russian::cName, Lazy<std::shared_ptr<Base>> ([]()
                                                                 {
    ...
    namespace ElectrumWords {
            ...
            std::string &languageName)
        {
            ...
            for (unsigned int i = 0; i < seed.size () / 3; i++) {
                ...
                /*!
                 * copy 4 bytes to position
                 */
                memcpy (dst.data + i * 4, &val, 4);
            }

            std::string wListCopy = words;
            if (seed.size () == seedLength / 2) {
                /*!
                 * if electrum 12-word seed, duplicate it
                 */
                memcpy (dst.data + 16, dst.data, 16);
                wListCopy += ' ';
                wListCopy += words;
            }
        ...
        void getLanguageList(std::vector<std::string> &languages)
        {
            ...
            std::transform (
                ...
                [](const Language::LanguageMap::value_type &pair)
                {
                    return pair.first;
                }
            );
