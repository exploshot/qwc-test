...
namespace {
    ...
    void checkOutputKey(const KeyDerivation &derivation,
                        const PublicKey &key,
                        size_t keyIndex,
                        size_t outputIndex,
    ...
    void findMyOutputs(const ITransactionReader &tx,
...
namespace CryptoNote {
    ...
    TransfersConsumer::TransfersConsumer(const CryptoNote::Currency &currency,
                                         INode &node,
    ...
    uint32_t TransfersConsumer::onNewBlocks(const CompleteBlock *blocks, uint32_t startHeight, uint32_t count)
    {
        ...
        try {
            ...
            m_logger (ERROR, BRIGHT_RED)
                << "Failed to process block transactions: failed to confirm transaction "
                << e.getTxHash ()
                <<
            ...
            m_logger (ERROR, BRIGHT_RED)
                << "Failed to process block transactions, exception: "
                << e.what ();
        } catch (...) {
            m_logger (ERROR, BRIGHT_RED)
                << "Failed to process block transactions, unknown exception";
        }

        if (processedBlockCount < count) {
            uint32_t detachIndex = startHeight + processedBlockCount;
            m_logger (ERROR, BRIGHT_RED)
                << "Not all block transactions are processed, fully processed block count: "
                << processedBlockCount
                << " of "
                << count
                <<
                ", last processed block hash "
                << (processedBlockCount > 0 ? blocks[processedBlockCount - 1].blockHash : Constants::NULL_HASH)
                <<
                ", detach block index "
                << detachIndex
    ...
    std::error_code createTransfers(const AccountKeys &account,
                                    ...
                                    Logging::LoggerRef &m_logger)
    {
        ...
        for (auto idx : outputs) {
            ...
            if (outType == TransactionTypes::OutputType::Key) {
                ...
                CryptoNote::generateKeyImageHelper (
                    account,
                    txPubKey,
                    idx,
                    in_ephemeral,
                ...
                if (transactions_hash_seen.find (tx.getTransactionHash ()) == transactions_hash_seen.end ()) {
                    if (public_keys_seen.find (out.key) != public_keys_seen.end ()) {
                        m_logger (WARNING, BRIGHT_RED)
                            << "A duplicate public key was found in "
                                               ...
                                               const PreprocessInfo &info)
    {
        ...
        if (someContainerUpdated) {
            m_logger (TRACE)
                << "Transaction updated some containers, hash "
                << tx.getTransactionHash ();
            ...
            m_logger (TRACE)
                << "Transaction doesn't updated any container, hash "
                << tx.getTransactionHash ();
        }
    }

    void TransfersConsumer::processOutputs(const TransactionBlockInfo &blockInfo,
