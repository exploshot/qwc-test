...
namespace CryptoNote {
                                                   ...
                                                   const std::string &indexesFilename)
    {
        ...
        if (resultCode != SQLITE_OK) {
            sqlite3_close (m_db);
            throw std::runtime_error ("Failed to load main chain storage from " +
                                      blocksFilename +
                                      ": " +
                                      sqlite3_errmsg (m_db));
        }
    ...
    RawBlock MainChainStorageSqlite::getBlockByIndex(const uint32_t index)
    {
        ...
        */
        ...
        int resultCode = sqlite3_prepare_v2 (m_db,
                                             "SELECT rawBlock FROM rawBlocks WHERE blockIndex = ? LIMIT 1",
                                             -1,
                                             &stmt,
                                             NULL);
        ...
        */
        while ((resultCode = sqlite3_step (stmt)) == SQLITE_ROW) {
            const std::string rawBlockString =
    ...
    uint32_t MainChainStorageSqlite::getBlockCount() const
    {
        ...
        int resultCode = sqlite3_prepare_v2 (m_db,
                                             "SELECT COUNT(*) AS blockCount FROM rawBlocks",
                                             -1,
                                             &stmt,
                                             NULL);
    ...
    {
        const int resultCode = sqlite3_exec (m_db,
    ...
    std::unique_ptr<IMainChainStorage> createSwappedMainChainStorageSqlite(const std::string &dataDir,
                                                                           const Currency &currency)
    {
        ...
        auto storage = std::make_unique<MainChainStorageSqlite> (blocksFilename.string () +
                                                                 ".sqlite3",
