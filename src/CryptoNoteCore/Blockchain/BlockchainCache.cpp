...
namespace CryptoNote {
        ...
        void splitGlobalIndexes(T &sourceContainer, T &destinationContainer, uint32_t splitBlockIndex, F lowerBoundFunction) 
        {
            for (auto it = sourceContainer.begin(); it != sourceContainer.end();) {
                auto newCacheOutputsIteratorStart =
                    lowerBoundFunction(it->second.outputs.begin(), 
                                      it->second.outputs.end(), 
                                      splitBlockIndex);

                auto &indexesForAmount = destinationContainer[it->first];
                auto newCacheOutputsCount =
                    static_cast<uint32_t>(std::distance(newCacheOutputsIteratorStart, 
                                                        it->second.outputs.end()));
                indexesForAmount.outputs.reserve(newCacheOutputsCount);

                indexesForAmount.startIndex = it->second.startIndex + 
                                              static_cast<uint32_t>(
                                                  it->second.outputs.size()) - 
                                                  newCacheOutputsCount;

                std::move(newCacheOutputsIteratorStart, 
                          it->second.outputs.end(), 
                          std::back_inserter(indexesForAmount.outputs));
                it->second.outputs.erase(newCacheOutputsIteratorStart, 
                                        it->second.outputs.end());
          ...
          storage(new BlockchainStorage(100)) 
    {
        if (parent == nullptr) {
            ...
            for (const TransactionOutput &output : 
                 genesisBlock.getBlock().baseTransaction.outputs) {
                minerReward += output.amount;
            }

            assert(minerReward > 0);

            uint64_t coinbaseTransactionSize = 
                getObjectBinarySize(genesisBlock.getBlock().baseTransaction);
        ...
        logger(Logging::DEBUGGING) 
            << "BlockchainCache with start block index: " 
            << startIndex 
            << " created";
    }
                                      ...
                                      RawBlock &&rawBlock) 
    {
        ...
        if (getBlockCount() == 0) {
            if (parent != nullptr) {
              cumulativeDifficulty = 
                  parent->getCurrentCumulativeDifficulty(cachedBlock.getBlockIndex() - 1);

              alreadyGeneratedCoins = 
                  parent->getAlreadyGeneratedCoins(cachedBlock.getBlockIndex() - 1);

              alreadyGeneratedTransactions = 
                  parent->getAlreadyGeneratedTransactions(cachedBlock.getBlockIndex() - 1);
            }
            ...
            cumulativeDifficulty = lastBlockInfo.cumulativeDifficulty + 
                                   blockDifficulty;
            alreadyGeneratedCoins = lastBlockInfo.alreadyGeneratedCoins + 
                                    generatedCoins;
            alreadyGeneratedTransactions = lastBlockInfo.alreadyGeneratedTransactions + 
        ...
        logger(Logging::DEBUGGING) 
            << "Added " 
            << validatorState.spentKeyImages.size() 
            << " spent key images";
        ...
        pushTransaction(CachedTransaction(std::move(baseTransaction)), 
                                          blockIndex, 
                                          transactionBlockIndex++);

        for (auto &cachedTransaction : cachedTransactions) {
            pushTransaction(cachedTransaction, 
                            blockIndex, 
                            transactionBlockIndex++);
        }
    ...
    PushedBlockInfo BlockchainCache::getPushedBlockInfo(uint32_t blockIndex) const 
    {
        ...
        if (blockIndex > startIndex) {
            ...
            pushedBlockInfo.blockDifficulty = cachedBlock.cumulativeDifficulty - 
                                              previousBlock.cumulativeDifficulty;
            pushedBlockInfo.generatedCoins = cachedBlock.alreadyGeneratedCoins - 
                                             previousBlock.alreadyGeneratedCoins;
        } else {
            if (parent == nullptr) {
                pushedBlockInfo.blockDifficulty = cachedBlock.cumulativeDifficulty;
                pushedBlockInfo.generatedCoins = cachedBlock.alreadyGeneratedCoins;
    ...
    */
    ...
    void BlockchainCache::splitSpentKeyImages(BlockchainCache &newCache, 
                                              uint32_t splitBlockIndex) 
    {
        /*!
        ...
        */
        ...
        newCache.spentKeyImages.get<BlockIndexTag>().insert(lowerBound, 
                                                            imagesIndex.end());
        imagesIndex.erase(lowerBound, imagesIndex.end());

        logger(Logging::DEBUGGING) 
            << "Spent key images split completed";
    }

    void BlockchainCache::splitTransactions(BlockchainCache &newCache, 
                                            uint32_t splitBlockIndex) 
    {
        ...
        newCache.transactions.get<BlockIndexTag>().insert(lowerBound, 
                                                          transactionsIndex.end());
        transactionsIndex.erase(lowerBound, transactionsIndex.end());

        logger(Logging::DEBUGGING) 
            << "Transactions split completed";
    }

    void BlockchainCache::removePaymentId(const Crypto::Hash &transactionHash, 
    ...
    void BlockchainCache::splitBlocks(BlockchainCache &newCache, 
                                      uint32_t splitBlockIndex) 
    {
        ...
        std::move(bound, 
                  blocksIndex.end(), 
                  std::back_inserter(newCache.blockInfos.get<BlockIndexTag>()));
        blocksIndex.erase(bound, blocksIndex.end());

        logger(Logging::DEBUGGING) 
            << "Blocks split completed";
    }

    void BlockchainCache::splitKeyOutputsGlobalIndexes(BlockchainCache &newCache, 
                                                       uint32_t splitBlockIndex) 
    {
        ...
        {
            return std::lower_bound(begin, end, splitBlockIndex, [](PackedOutIndex outputIndex, 
                                                                    uint32_t splitIndex) 
            {
                /*!
        ...
        splitGlobalIndexes(keyOutputsGlobalIndexes, 
                           newCache.keyOutputsGlobalIndexes, 
                           splitBlockIndex, 
                           lowerBoundFunction);
        logger(Logging::DEBUGGING) 
            << "Key output global indexes split successfully completed";
    }

    void BlockchainCache::addSpentKeyImage(const Crypto::KeyImage &keyImage, 
                                           uint32_t blockIndex) 
    {
        /*!
            Changed from "assert(!checkIfSpent(keyImage, blockIndex));"
        ...
        */
        assert(!checkIfSpent(keyImage, blockIndex - 1)); 
    ...
    void BlockchainCache::pushTransaction(const CachedTransaction &cachedTransaction, 
                                          uint32_t blockIndex,
                                          uint16_t transactionInBlockIndex) 
    {
        ...
        for (auto &output : tx.outputs) {
            ...
            if (output.target.type() == typeid(KeyOutput)) {
                transactionCacheInfo.globalIndexes.push_back(
                    insertKeyOutputToGlobalIndex(output.amount, 
                                                 poi, blockIndex)
                );
            }
        }
        ...
        logger(Logging::DEBUGGING) 
            << "Payment id found: " 
            << paymentIdTransactionHash.paymentId;
    ...
    uint32_t BlockchainCache::insertKeyOutputToGlobalIndex(uint64_t amount, 
                                                           ...
                                                           uint32_t blockIndex) 
    {
        ...
        if (pair.second && parent != nullptr) {
            ...
            logger(Logging::DEBUGGING) 
                << "Key output count for amount " 
                << amount 
                << " requested from parent. Returned count: " 
                << indexEntry.startIndex;
        }

        return indexEntry.startIndex + static_cast<uint32_t>(indexEntry.outputs.size()) - 1;
    }

    bool BlockchainCache::checkIfSpent(const Crypto::KeyImage &keyImage, 
    ...
    uint32_t BlockchainCache::getBlockIndex(const Crypto::Hash &blockHash) const 
    {
        ...
        return static_cast<uint32_t>(
            std::distance(
                blockInfos.get<BlockIndexTag>().begin(), 
                rndIt
            )
        ) + startIndex;
    }
    ...
    std::vector<Crypto::Hash> BlockchainCache::getBlockHashes(uint32_t startBlockIndex, 
    ...
    size_t BlockchainCache::getKeyOutputsCountForAmount(uint64_t amount, 
                                                        uint32_t blockIndex) const 
    {
        ...
        return it->second.startIndex + static_cast<size_t>(std::distance(it->second.outputs.begin(), 
                                                                         lowerBound));
    }
    ...
    uint32_t BlockchainCache::getTimestampLowerBoundBlockIndex(uint64_t timestamp) const 
    {
        ...
        if (index.front().timestamp < timestamp) {
            ...
            */
            auto bound = std::lower_bound(index.begin(), 
                                          index.end(), 
                                          timestamp,
                                          [](
    ...
    std::vector<RawBlock> BlockchainCache::getNonEmptyBlocks(const uint64_t startHeight,
    ...
    std::vector<RawBlock> BlockchainCache::getBlocksByHeight(const uint64_t startHeight, 
    ...
    {
        return index < startIndex ? parent->getBlockByIndex(index) : 
                       storage->getBlockByIndex(index - startIndex);
    }

    BinaryArray BlockchainCache::getRawTransaction(uint32_t index, 
    ...
    std::vector<BinaryArray> 
    BlockchainCache::getRawTransactions(const std::vector<Crypto::Hash> &requestedTransactions,
                                             ...
                                             std::vector<Crypto::Hash> &missedTransactions) const 
    {
        ...
        for (const auto &transactionHash : requestedTransactions) {
            ...
            foundTransactions.push_back(getRawTransaction(it->blockIndex, 
                                                          it->transactionIndex));
        }
    }
    ...
    {
        assert(std::find(children.begin(), 
                         children.end(), 
                         child) == children.end());
    ...
    void BlockchainCache::serialize(ISerializer &s) 
    {
        ...
        if (s.type() == ISerializer::OUTPUT) {
            writeSequence<CachedTransactionInfo>(transactions.begin(), 
                                                 transactions.end(), 
                                                 "transactions", 
                                                 s);
            writeSequence<SpentKeyImage>(spentKeyImages.begin(), 
                                         spentKeyImages.end(), 
                                         "spent_key_images", 
                                         s);
            writeSequence<CachedBlockInfo>(blockInfos.begin(), 
                                           blockInfos.end(), 
                                           "block_hash_indexes", 
                                           s);
            writeSequence<PaymentIdTransactionHashPair>(paymentIds.begin(), 
                                                        paymentIds.end(), 
                                                        "payment_id_indexes", 
                                                        s);
            ...
            readSequence<CachedTransactionInfo>(std::inserter(restoredTransactions, 
                                                restoredTransactions.end()), 
                                                "transactions", 
                                                s);
            readSequence<SpentKeyImage>(std::inserter(restoredSpentKeyImages, 
                                                      restoredSpentKeyImages.end()), 
                                                      "spent_key_images", 
                                                      s);
            readSequence<CachedBlockInfo>(std::back_inserter(restoredBlockHashIndex), 
                                                             "block_hash_indexes", 
                                                             s);
            readSequence<PaymentIdTransactionHashPair>(std::inserter(restoredPaymentIds, 
                                                                     restoredPaymentIds.end()), 
                                                                     "payment_id_indexes", 
                                                                     s);
    ...
    bool BlockchainCache::isTransactionSpendTimeUnlocked(uint64_t unlockTime, 
                                                         uint32_t blockIndex) const 
    {
        ...
        */
        return static_cast<uint64_t>(time(nullptr)) + 
                                     currency.lockedTxAllowedDeltaSeconds() >= 
                                     unlockTime;
    }

    ExtractOutputKeysResult 
    BlockchainCache::extractKeyOutputKeys(uint64_t amount,
    ...
    std::vector<uint32_t> BlockchainCache::getRandomOutsByAmount(Amount amount, 
                                                                 size_t count, 
                                                                 uint32_t blockIndex) const 
    {
        ...
        */
        ...
        /*!
            ...
            that is unlocked 
        */
        const auto end = std::find_if(outs.rbegin(), 
                                      outs.rend(), 
                                      [&](const auto index)
        {
        ...
        */
        if (outputs.size() < count && parent != nullptr) {
            const auto prevs = parent->getRandomOutsByAmount(amount, 
                                                             count - outputs.size(), 
                                                             blockIndex);

            std::copy(prevs.begin(), 
                      prevs.end(), 
                                          ...
                                          std::vector<Crypto::PublicKey> &publicKeys) const 
    {
        ...
        /*!
            sorted
        */
        assert(std::is_sorted(globalIndexes.begin(), 
                              globalIndexes.end()));
        /*!
            unique
        */
        assert(std::adjacent_find(globalIndexes.begin(), 
                                  globalIndexes.end()) == 
                                  globalIndexes.end());
    ...
    ExtractOutputKeysResult
    BlockchainCache::extractKeyOtputReferences(uint64_t amount, 
                                               Common::ArrayView<uint32_t> globalIndexes,
    ...
    ExtractOutputKeysResult BlockchainCache::extractKeyOutputs(uint64_t amount, 
                                                               uint32_t blockIndex, 
                                                               Common::ArrayView<uint32_t> globalIndexes, 
                                                                   ...
                                                                   uint32_t globalIndex)> pred) const 
    {
        ...
        */
        ...
        if (globalIndexesIterator == keyOutputsGlobalIndexes.end() || blockIndex < startIndex) {
            return parent != nullptr ? parent->extractKeyOutputs(amount, 
                                                                 blockIndex, 
                                                                 globalIndexes, 
                                                                 std::move(pred))
        ...
        auto parentIndexesIterator = std::lower_bound(globalIndexes.begin(), 
                                                      globalIndexes.end(), 
                                                      startGlobalIndex);
        ...
        if (parentIndexesIterator != globalIndexes.begin()) {
            assert(parent != nullptr);
            auto result = parent->extractKeyOutputs(amount, 
                                                    blockIndex, 
                                                    globalIndexes.head(
                                                        parentIndexesIterator - 
                                                        globalIndexes.begin()), 
                                                        pred);
        ...
        for (auto globalIndex : myGlobalIndexes) {
            if (globalIndex - startGlobalIndex >= outputs.size()) {
                logger(Logging::DEBUGGING) 
                    << "Couldn't extract key output for amount " 
                    << amount 
                    << " with global index " 
                    << globalIndex
    ...
    BlockchainCache::getTransactionHashesByPaymentId(const Crypto::Hash &paymentId) const 
    {
        ...
        logger(Logging::DEBUGGING) 
            ...
            << " transactions with payment id " 
            << paymentId;
        return transactionHashes;
    }

    std::vector<Crypto::Hash> 
    BlockchainCache::getBlockHashesByTimestamps(uint64_t timestampBegin, 
                                                size_t secondsCount) const 
    {
        ...
        logger(Logging::DEBUGGING) 
            << "Found " 
            << blockHashes.size() 
            << " within timestamp interval "
            << "[" 
            << timestampBegin 
            << ":" 
            << (timestampBegin + secondsCount) 
            << "]";
        return blockHashes;
    }

    ExtractOutputKeysResult 
    BlockchainCache::extractKeyOtputIndexes(uint64_t amount,
                                            Common::ArrayView<uint32_t> globalIndexes,
    ...
    std::vector<uint64_t> BlockchainCache::getLastTimestamps(size_t count, 
                                                             uint32_t blockIndex,
    ...
    std::vector<uint64_t> 
    BlockchainCache::getLastUnits(size_t count, 
                                  uint32_t blockIndex, 
                                  ...
                                  std::function<uint64_t(const CachedBlockInfo&)> pred) const 
    {
        ...
        if (realCount < count && parent != nullptr) {
            result =
                parent->getLastUnits(count - realCount, 
                                     std::min(blockIndex, 
                                     parent->getTopBlockIndex()), 
                                     useGenesis, 
                                     pred);
        }

        std::transform(std::next(blocksIndex.begin(), from), 
    ...
    std::vector<uint64_t> BlockchainCache::getLastBlocksSizes(size_t count, 
                                                              uint32_t blockIndex,
                                                              UseGenesis useGenesis) const 
    {
        return getLastUnits(count, 
    ...
    uint64_t BlockchainCache::getDifficultyForNextBlock(uint32_t blockIndex) const 
    {
        ...
        auto timestamps = getLastTimestamps(currency.difficultyBlocksCountByHeight(blockIndex), 
                                            blockIndex, 
                                            skipGenesisBlock);
        auto commulativeDifficulties =  getLastCumulativeDifficulties(currency.difficultyBlocksCountByHeight(
                                                                          blockIndex
                                                                      ), 
                                                                      blockIndex, 
                                                                      skipGenesisBlock);
    ...
    uint64_t BlockchainCache::getCurrentCumulativeDifficulty(uint32_t blockIndex) const 
    {
        assert(!blockInfos.empty());
        assert(blockIndex <= getTopBlockIndex());
        return blockInfos.get<BlockIndexTag>().at(blockIndex - 
                                                  startIndex).cumulativeDifficulty;
    }
    ...
    uint64_t BlockchainCache::getAlreadyGeneratedCoins(uint32_t blockIndex) const 
    {
        ...
        return blockInfos.get<BlockIndexTag>().at(blockIndex - 
                                                  startIndex).alreadyGeneratedCoins;
    }
    ...
    uint64_t BlockchainCache::getAlreadyGeneratedTransactions(uint32_t blockIndex) const 
    {
        ...
        return blockInfos.get<BlockIndexTag>().at(blockIndex - 
                                                  startIndex).alreadyGeneratedTransactions;
    }

    std::vector<uint64_t> 
    BlockchainCache::getLastCumulativeDifficulties(size_t count, 
                                                   uint32_t blockIndex,
                                                   UseGenesis useGenesis) const 
    {
    ...
    uint8_t BlockchainCache::getBlockMajorVersionForHeight(uint32_t height) const 
    {
        ...
        upgradeManager.addMajorBlockVersion(
