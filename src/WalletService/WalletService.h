...
namespace PaymentService {
    ...
    public:
        ...
        std::error_code createAddress(const std::string &spendSecretKeyText,
        ...
        std::error_code getBlockHashes(uint32_t firstBlockIndex,
                                       uint32_t blockCount,
        ...
        std::error_code getTransactionHashes(const std::vector <std::string> &addresses,
        ...
        std::error_code createDelayedTransaction(CreateDelayedTransaction::Request &request,
        ...
        std::error_code getStatus(uint32_t &blockCount,
                                  uint32_t &knownBlockCount,
                                  uint64_t &localDaemonBlockCount,
                                  std::string &lastBlockHash,
                                  uint32_t &peerCount);
        std::error_code sendFusionTransaction(uint64_t threshold,
                                              uint32_t anonymity,
                                              const std::vector <std::string> &addresses,
                                              const std::string &destinationAddress,
                                              std::string &transactionHash);
        std::error_code estimateFusion(uint64_t threshold,
                                       const std::vector <std::string> &addresses,
                                       uint32_t &fusionReadyCount,
                                       uint32_t &totalOutputCount);
        std::error_code createIntegratedAddress(const std::string &address,
                                                const std::string &paymentId,
    ...
    private:
        ...
        std::vector <TransactionHashesInBlockRpcInfo> getRpcTransactionHashes(const Crypto::Hash &blockHash,
                                                                              size_t blockCount,
                                                                              const TransactionsInBlockInfoFilter &filter) const;
        std::vector <TransactionHashesInBlockRpcInfo> getRpcTransactionHashes(uint32_t firstBlockIndex,
                                                                              size_t blockCount,
                                                                              const TransactionsInBlockInfoFilter &filter) const;

        std::vector <TransactionsInBlockRpcInfo> getRpcTransactions(const Crypto::Hash &blockHash,
                                                                    size_t blockCount,
                                                                    const TransactionsInBlockInfoFilter &filter) const;
        std::vector <TransactionsInBlockRpcInfo> getRpcTransactions(uint32_t firstBlockIndex,
                                                                    size_t blockCount,
                                                                    const TransactionsInBlockInfoFilter &filter) const;
