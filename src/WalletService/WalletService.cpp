...
namespace PaymentService {
            ...
            const uint64_t height)
        {
            ...
            if (mixin < minMixin) {
                str
                    << "Mixin of "
                    << mixin
                    << " under minimum mixin threshold of "
                    << minMixin;
                ...
                str
                    << "Mixin of "
                    << mixin
                    << " above maximum mixin threshold of "
                    << maxMixin;
    ...
    namespace {
                                                                      ...
                                                                      Logging::LoggerRef logger)
        {
             ...
             */
            if (!Tools::Base58::decodeAddress (integratedAddr, prefix, decoded)) {
                throw std::system_error (make_error_code (CryptoNote::error::BAD_ADDRESS));
            }
                           ...
                           System::Dispatcher &dispatcher)
    {
        ...
        if (conf.secretSpendKey.empty () && conf.secretViewKey.empty () && conf.mnemonicSeed.empty ()) {
            ...
            Crypto::CryptoOps::generateViewFromSpend (spendKey.secretKey, private_view_key);
            ...
            log (Logging::INFO, Logging::BRIGHT_WHITE)
                << "New wallet is generated. Address: "
                << address;
            ...
            Crypto::CryptoOps::generateViewFromSpend (private_spend_key, private_view_key);

            wallet->initializeWithViewKey (conf.walletFile,
                                           conf.walletPassword,
                                           private_view_key,
                                           conf.scanHeight,
                                           false);
            ...
            if (conf.secretSpendKey.empty () || conf.secretViewKey.empty ()) {
                log (Logging::ERROR, Logging::BRIGHT_RED)
                    << "Need both secret spend key and secret view key.";
                return;
            } else {
                log (Logging::INFO, Logging::BRIGHT_WHITE)
                    << "Attemping to import wallet from keys";
    ...
    void WalletService::getNodeFee()
    {
        logger (Logging::DEBUGGING)
            <<
            "Trying to retrieve node fee information."
            << std::endl;
        ...
        if (!m_node_address.empty () && m_node_fee != 0) {
            ...
            stream
                << std::setfill ('0')
                << std::setw (CryptoNote::parameters::CRYPTONOTE_DISPLAY_DECIMAL_POINT)
                << cents;
    ...
    std::error_code WalletService::getMnemonicSeed(const std::string &address, std::string &mnemonicSeed)
    {
        try {
            ...
            Crypto::CryptoOps::generateViewFromSpend (key.secretKey, deterministic_private_view_key);

            bool deterministic_private_keys = deterministic_private_view_key == viewKey.secretKey;

            if (deterministic_private_keys) {
                mnemonicSeed = Mnemonics::PrivateKeyToMnemonic (key.secretKey, "English");
            } else {
                /*!
    ...
    std::error_code WalletService::sendTransaction(SendTransaction::Request &request, std::string &transactionHash)
    {
        try {
             ...
             */
            std::transform (request.paymentId.begin (),
                            request.paymentId.end (),
                            request.paymentId.begin (),
                            ::toupper);
    ...
    std::error_code WalletService::deleteDelayedTransaction(const std::string &transactionHash)
    {
        try {
            ...
            /*!
             * validate transactionHash parameter
             */
            parseHash (transactionHash, logger);
                                                                   ...
                                                                   std::vector <std::string> &transactionHashes)
    {
        try {
            ...
            std::vector <CryptoNote::WalletTransactionWithTransfers>
                transactions = wallet.getUnconfirmedTransactions ();
     ...
     */
    std::error_code WalletService::getStatus(uint32_t &blockCount,
                                             ...
                                             uint32_t &peerCount)
    {
        try {
            ...
            System::RemoteContext <std::tuple<uint32_t, uint64_t, uint32_t>> remoteContext (dispatcher, [this]()
            {
                 ...
                 */
                return std::make_tuple (node.getKnownBlockCount (),
                                        node.getNodeHeight (),
    ...
    std::error_code WalletService::sendFusionTransaction(uint64_t threshold,
                                                         uint32_t anonymity,
    ...
    std::error_code WalletService::createIntegratedAddress(const std::string &address,
                                                           const std::string &paymentId,
                                                           std::string &integratedAddress)
    {
        try {
            ...
            logger (Logging::WARNING, Logging::BRIGHT_YELLOW)
                << "Error while creating integrated address: "
                << x.what ();
            return x.code ();
        }
         ...
         */
        integratedAddress = Tools::Base58::encodeAddress
            (
                CryptoNote::parameters::CRYPTONOTE_PUBLIC_ADDRESS_BASE58_PREFIX,
                paymentId + keys
            );
    ...
    std::vector <TransactionHashesInBlockRpcInfo> WalletService::getRpcTransactionHashes(const Crypto::Hash &blockHash,
                                                                                         size_t blockCount,
    ...
    std::vector <TransactionHashesInBlockRpcInfo> WalletService::getRpcTransactionHashes(uint32_t firstBlockIndex,
                                                                                         size_t blockCount,
                                                                                         const TransactionsInBlockInfoFilter &filter) const
    {
        std::vector <CryptoNote::TransactionsInBlockInfo>
            allTransactions = getTransactions (firstBlockIndex, blockCount);
        std::vector <CryptoNote::TransactionsInBlockInfo>
            filteredTransactions = filterTransactions (allTransactions, filter);
        return convertTransactionsInBlockInfoToTransactionHashesInBlockRpcInfo (filteredTransactions);
    }

    std::vector <TransactionsInBlockRpcInfo> WalletService::getRpcTransactions(const Crypto::Hash &blockHash,
                                                                               size_t blockCount,
                                                                               const TransactionsInBlockInfoFilter &filter) const
    {
        std::vector <CryptoNote::TransactionsInBlockInfo> allTransactions = getTransactions (blockHash, blockCount);
    ...
    std::vector <TransactionsInBlockRpcInfo> WalletService::getRpcTransactions(uint32_t firstBlockIndex,
                                                                               size_t blockCount,
                                                                               const TransactionsInBlockInfoFilter &filter) const
    {
        std::vector <CryptoNote::TransactionsInBlockInfo> allTransactions = getTransactions (firstBlockIndex,
                                                                                             blockCount);
        std::vector <CryptoNote::TransactionsInBlockInfo> filteredTransactions = filterTransactions (allTransactions,
                                                                                                     filter);
        return convertTransactionsInBlockInfoToTransactionsInBlockRpcInfo (filteredTransactions);
