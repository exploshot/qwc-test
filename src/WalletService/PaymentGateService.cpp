...
bool PaymentGateService::init(int argc, char **argv)
{
    ...
    if (!config.serviceConfig.serverRoot.empty ()) {
        ...
        log (Logging::INFO)
            ...
            << config.serviceConfig.serverRoot;
    }
...
{
    log (Logging::INFO)
        << "Starting Payment Gate with remote node, timeout: "
...
void PaymentGateService::runWalletService(const CryptoNote::Currency &currency, CryptoNote::INode &node)
{
    ...
    std::unique_ptr <CryptoNote::WalletGreen> wallet (new CryptoNote::WalletGreen (*dispatcher,
                                                                                   currency,
                                                                                   node,
                                                                                   logger));

    service = new PaymentService::WalletService (currency,
                                                 *dispatcher,
                                                 node,
                                                 *wallet,
                                                 *wallet,
