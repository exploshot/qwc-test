...
namespace {
    ...
    uint64_t getSignaturesCount(const TransactionInput &input)
    {
        struct txin_signature_size_visitor: public boost::static_visitor<uint64_t>
        {
            ...
            uint64_t operator()(const MultisignatureInput &txin) const
            {
                return txin.signatureCount;
            }
        };
    ...
    struct BinaryVariantTagGetter: boost::static_visitor<uint8_t>
    {
        ...
        uint8_t operator()(const CryptoNote::MultisignatureInput)
        {
            return 0x3;
        }
        ...
        uint8_t operator()(const CryptoNote::MultisignatureOutput)
        {
            return 0x3;
        }
    ...
    bool serializeVarintVector(std::vector <uint32_t> &vector,
...
namespace CryptoNote {
    ...
    void serialize(ParentBlockSerializer &pbs, ISerializer &serializer)
    {
        ...
        uint64_t branchSize = Crypto::treeDepth (pbs.m_parentBlock.transactionCount);
        if (serializer.type () == ISerializer::OUTPUT) {
