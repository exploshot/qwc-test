...
namespace CryptoNote {
    ...
    bool TransfersContainer::addTransaction(const TransactionBlockInfo &block,
                                            ...
                                            const std::vector <TransactionOutputInformationIn> &transfers)
    {
        ...
        try {
            ...
            if (m_transactions.count (tx.getTransactionHash ()) == 0) {
                m_logger (ERROR, BRIGHT_RED)
                    << "Failed to add transaction, remove transaction transfers, block "
                    << block.height
                    <<
                    ", transaction hash "
                    << tx.getTransactionHash ();
     ...
     * pre m_mutex is locked.
     */
    bool TransfersContainer::addTransactionOutputs(const TransactionBlockInfo &block,
                                                   const ITransactionReader &tx,
                                                   const std::vector <TransactionOutputInformationIn> &transfers)
    {
            ...
            if (inputType == TransactionTypes::InputType::Key) {
                ...
                if (std::distance (spentRange.first, spentRange.second) > 0) {
                    ...
                    m_logger (ERROR, BRIGHT_RED)
                        ...
                        <<
                        ": hash "
                        << spentOutput.spendingTransactionHash
                        <<
                        ", block "
                        << spentOutput.spendingBlock.height
                        <<
                        ", input "
                        ...
                        <<
                        ": hash "
                        << spentOutput.transactionHash
                        <<
                        ", block "
                        << spentOutput.blockHeight
                        <<
                        ", transaction index "
                        << spentOutput.transactionIndex
                        <<
                        ", output "
                        << spentOutput.outputInTransaction
                        <<
                        ", amount "
                ...
                auto
                    availableOutputsRange = outputDescriptorIndex.equal_range (SpentOutputDescriptor (&input.keyImage));
                                                      ...
                                                      const std::vector <uint32_t> &globalIndices)
    {
        ...
        try {
            ...
            m_logger (ERROR, BRIGHT_RED)
                << "markTransactionConfirmed failed: "
                << e.what ()
                << ", rollback changes, block index "
                << block.height
                <<
                ", tx "
                << transactionHash;
    ...
    bool TransfersContainer::getTransactionInformation(const Hash &transactionHash,
    ...
    void TransfersContainer::save(std::ostream &os)
    {
        ...
        writeSequence<TransactionOutputInformationEx> (m_unconfirmedTransfers.begin (),
                                                       m_unconfirmedTransfers.end (),
                                                       "unconfirmedTransfers",
                                                       s);
        writeSequence<TransactionOutputInformationEx> (m_availableTransfers.begin (),
