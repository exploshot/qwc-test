...
Error validateFusionTransaction(const uint64_t mixin,
...
Error validateTransaction(const std::vector<std::pair<std::string, uint64_t>> destinations,
                          ...
                          const uint64_t currentHeight)
{
     ...
     */
    if (Error error = validateAmount (destinations,
                                      fee,
                                      subWalletsToTakeFrom,
                                      subWallets,
                                      currentHeight); error != SUCCESS) {
        return error;
    }
...
{
    const bool valid = scCheck (reinterpret_cast<const unsigned char *>(&privateViewKey)) == 0;
...
{
    const bool valid = Crypto::checkKey (publicKey);
...
Error validateAmount(const std::vector<std::pair<std::string, uint64_t>> destinations,
                     const uint64_t fee,
                     const std::vector<std::string> subWalletsToTakeFrom,
                     const std::shared_ptr<SubWallets> subWallets,
                     const uint64_t currentHeight)
    ...
    const bool integratedAddressesAllowed)
{
    for (auto &address : addresses) {
        ...
        if (address.length () != WalletConfig::standardAddressLength &&
            ...
            stream
                << "The address given is the wrong length. It should be "
                << WalletConfig::standardAddressLength
                << " chars or "
                << WalletConfig::integratedAddressLength
                << " chars, but "
                << "it is "
                << address.length ()
                << " chars.";
        ...
        if (address.length () == WalletConfig::integratedAddressLength) {
            ...
            if (!Tools::Base58::decodeAddress (address, ignore, decoded)) {
