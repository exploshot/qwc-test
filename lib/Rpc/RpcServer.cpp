...
namespace CryptoNote {
    ...
    std::unordered_map<std::string, RpcServer::RpcHandler<RpcServer::HandlerFunction>> RpcServer::s_handlers = {
        ...
        {
            "/getheight",
            {jsonMethod<COMMAND_RPC_GET_HEIGHT> (&RpcServer::onGetHeight), true}
        },
        {
            "/feeinfo",
            {jsonMethod<COMMAND_RPC_GET_FEE_ADDRESS> (&RpcServer::onGetFeeInfo), true}
        },
        {
            "/getpeers",
            {jsonMethod<COMMAND_RPC_GET_PEERS> (&RpcServer::onGetPeers), true}
        },
        {
            "/info",
            {jsonMethod<COMMAND_RPC_GET_INFO> (&RpcServer::onGetInfo), true}
        ...
        {
            "/queryblocksdetailed",
            {jsonMethod<COMMAND_RPC_QUERY_BLOCKS_DETAILED> (&RpcServer::onQueryBlocksDetailed), false}
        },
        {
            "/getwalletsyncdata",
            {jsonMethod<COMMAND_RPC_GET_WALLET_SYNC_DATA> (&RpcServer::onGetWalletSyncData), false}
        },
        {
            "/get_o_indexes",
            {jsonMethod<COMMAND_RPC_GET_TX_GLOBAL_OUTPUTS_INDEXES> (&RpcServer::onGetIndexes), false}
        },
        {
            "/getrandom_outs",
    ...
    bool RpcServer::processJsonRpcRequest(const HttpRequest &request, HttpResponse &response)
    {
        ...
        try {
            ...
            static std::unordered_map<std::string, RpcServer::RpcHandler<JsonMemberMethod>> jsonRpcHandlers = {
                ...
                {"f_on_transactions_pool_json", {makeMemberMethod (&RpcServer::fOnTransactionsPoolJson), false}},
    ...
    {
        return m_p2p.getPayloadObject ().isSynchronized ();
    }
    ...
    bool RpcServer::onGetWalletSyncData(const COMMAND_RPC_GET_WALLET_SYNC_DATA::request &req,
                                        COMMAND_RPC_GET_WALLET_SYNC_DATA::response &res)
    {
        const bool success = m_core.getWalletSyncData (req.blockIds,
                                                       req.startHeight,
                                                       req.startTimestamp,
                                                       req.blockCount,
                                       ...
                                       COMMAND_RPC_GET_RANDOM_OUTPUTS_FOR_AMOUNTS::response &res)
    {
        ...
        for (uint64_t amount : req.amounts) {
            ...
            if (globalIndexes.size () != req.outs_count) {
                logger (ERROR)
                    << "Failed to get enough matching outputs for amount "
                    << amount
                    << " ("
    ...
    bool RpcServer::onGetInfo(const COMMAND_RPC_GET_INFO::request &req, COMMAND_RPC_GET_INFO::response &res)
    {
        ...
        res.white_peerlist_size = m_p2p.getPeerlistManager ().getWhitePeersCount ();
    ...
    bool RpcServer::onSendRawTx(const COMMAND_RPC_SEND_RAW_TX::request &req, COMMAND_RPC_SEND_RAW_TX::response &res)
    {
        ...
        Crypto::Hash transactionHash = Crypto::CnFastHash (transactions.back ().data (), transactions.back ().size ());
        logger (DEBUGGING)
            << "transaction "
            << transactionHash
            << " came in onSendRawTx";
        ...
        if (!m_core.addTransactionToPool (transactions.back ())) {
            logger (DEBUGGING)
                << "[onSendRawTx]: tx verification failed";
            res.status = "Failed";
            return true;
        }
    ...
    bool RpcServer::onGetFeeInfo(const COMMAND_RPC_GET_FEE_ADDRESS::request &req,
     ...
     * JSON RPC methods
     */
    bool RpcServer::fOnBlocksListJson(const F_COMMAND_RPC_GET_BLOCKS_LIST::request &req,
    ...
    bool RpcServer::fOnBlockJson(const F_COMMAND_RPC_GET_BLOCK_DETAILS::request &req,
                                 F_COMMAND_RPC_GET_BLOCK_DETAILS::response &res)
    {
        ...
        BlockHeaderResponse block_header;
         ...
         */
        FTransactionShortResponse transaction_short;
    ...
    bool RpcServer::fOnTransactionJson(const F_COMMAND_RPC_GET_TRANSACTION_DETAILS::request &req,
    ...
    bool RpcServer::fOnTransactionsPoolJson(const F_COMMAND_RPC_GET_POOL::request &req,
    ...
    bool RpcServer::fGetMixin(const Transaction &transaction, uint64_t &mixin)
    {
        mixin = 0;
    ...
    bool
    RpcServer::onGetBlockCount(const COMMAND_RPC_GETBLOCKCOUNT::request &req, COMMAND_RPC_GETBLOCKCOUNT::response &res)
    {
    ...
    bool
    RpcServer::onGetBlockHash(const COMMAND_RPC_GETBLOCKHASH::request &req, COMMAND_RPC_GETBLOCKHASH::response &res)
    {
        if (req.size () != 1) {
    ...
    bool RpcServer::onGetBlockTemplate(const COMMAND_RPC_GETBLOCKTEMPLATE::request &req,
                                       COMMAND_RPC_GETBLOCKTEMPLATE::response &res)
    {
        ...
        if (tx_pub_key == Constants::NULL_PUBLIC_KEY) {
            logger (ERROR)
                << "Failed to find tx pub key in coinbase extra";
            throw JsonRpc::JsonRpcError{CORE_RPC_ERROR_CODE_INTERNAL_ERROR,
        ...
        if (0 < req.reserve_size) {
            res.reserved_offset =
                slowMemMem ((void *) block_blob.data (), block_blob.size (), &tx_pub_key, sizeof (tx_pub_key));
            ...
            if (res.reserved_offset + req.reserve_size > block_blob.size ()) {
                logger (ERROR)
                    << "Failed to calculate offset for reserved bytes";
                throw JsonRpc::JsonRpcError{CORE_RPC_ERROR_CODE_INTERNAL_ERROR,
                                            "Internal error: failed to create block template"};
            }
        } else {
                                         ...
                                         COMMAND_RPC_GET_LAST_BLOCK_HEADER::response &res)
    {
        ...
        fillBlockHeaderResponse (topBlock,
                                 false,
                                 m_core.getTopBlockIndex (),
                                 m_core.getTopBlockHash (),
                                 res.block_header);
        res.status = CORE_RPC_STATUS_OK;
        return true;
    }
    ...
    {
        Hash blockHash;
        if (!parseHash256 (req.hash, blockHash)) {
            throw JsonRpc::JsonRpcError{
                CORE_RPC_ERROR_CODE_WRONG_PARAM,
                "Failed to parse hex representation of block hash. Hex = " + req.hash + '.'};
        }
                                               ...
                                               JsonRpc::JsonRpcError &error_resp)
    {
         ...
         */
        const uint64_t bc_height = m_core.getCurrentBlockchainHeight ();
