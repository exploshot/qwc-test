 ...
 * @param newWallet
 */
SubWallets::SubWallets(const Crypto::SecretKey privateViewKey,
                       const std::string address,
                       const uint64_t scanHeight,
                       const bool newWallet)
    : m_privateViewKey (privateViewKey),
    ...
    const uint64_t scanHeight)
{
    ...
    Crypto::secretKeyToPublicKey (m_privateViewKey, publicViewKey);
...
std::tuple <uint64_t, uint64_t> SubWallets::getMinInitialSyncStart() const
{
    ...
    auto minElementByHeight = *std::min_element (m_subWallets.begin (), m_subWallets.end (),
                                                 [](const auto &lhs, const auto &rhs)
                                                 {
                                                     return lhs.second.syncStartHeight ()
                                                            < rhs.second.syncStartHeight ();
                                                 });
...
void SubWallets::addUnconfirmedTransaction(const WalletTypes::Transaction tx)
{
    ...
    if (it2 != m_lockedTransactions.end ()) {
        ...
        stream
            ...
            << tx.hash
            << " was added to the wallet twice! Terminating.";
...
SubWallets::getTransactionInputsForAmount(const uint64_t amount,
...
SubWallets::getFusionTransactionInputs(const bool takeFromAll,
                                       std::vector <Crypto::PublicKey> subWalletsToTakeFrom,
 ...
 * @param spendHeight
 */
void SubWallets::markInputAsSpent(const Crypto::KeyImage keyImage,
 ...
 * @param publicKey
 */
void SubWallets::markInputAsLocked(const Crypto::KeyImage keyImage,
                                   const Crypto::PublicKey publicKey)
{
    /*!
...
void SubWallets::storeTxPrivateKey(const Crypto::SecretKey txPrivateKey,
...
void SubWallets::convertSyncTimestampToHeight(const uint64_t timestamp,
