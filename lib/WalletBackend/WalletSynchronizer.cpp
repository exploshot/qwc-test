 ...
 */
WalletSynchronizer::WalletSynchronizer(const std::shared_ptr <Nigel> daemon,
    ...
    const std::vector <std::tuple<Crypto::PublicKey, WalletTypes::TransactionInput>> &ourInputs)
{
    ...
    if (walletHeight >= block.blockHeight && block.blockHeight != 0) {
        Logger::logger.log (
            "Blockchain forked, resolving... (Old height: "
            + std::to_string (walletHeight)
            + ", new height: "
...
BlockScanTmpInfo WalletSynchronizer::processBlockTransactions(const WalletTypes::WalletBlockInfo &block,
                                                              const std::vector <std::tuple<Crypto::PublicKey,
                                                                                            WalletTypes::TransactionInput>> &inputs) const
{
...
std::optional <WalletTypes::Transaction>
WalletSynchronizer::processCoinbaseTransaction(const WalletTypes::WalletBlockInfo &block,
                                               const std::vector <std::tuple<Crypto::PublicKey,
                                                                             WalletTypes::TransactionInput>> &inputs) const
{
...
WalletSynchronizer::processTransaction(const WalletTypes::WalletBlockInfo &block,
                                       const std::vector <std::tuple<Crypto::PublicKey,
                                                                     WalletTypes::TransactionInput>> &inputs,
                                              ...
                                              const uint64_t blockHeight) const
{
    ...
    Crypto::generateKeyDerivation (rawTX.transactionPublicKey, m_privateViewKey, derivation);
    ...
    for (const auto output : rawTX.keyOutputs) {
        ...
        Crypto::underivePublicKey (derivation, outputIndex, output.key, derivedSpendKey);
 ...
 * separate function, so everything in the constructor gets initialized,
