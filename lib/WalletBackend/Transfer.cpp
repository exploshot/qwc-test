...
namespace SendTransaction {
                                                                   ...
                                                                   const std::shared_ptr <SubWallets> subWallets)
    {
         ...
         */
        Error error = validateFusionTransaction (mixin,
                                                 addressesToTakeFrom,
                                                 destination,
                                                 subWallets,
                                                 daemon->networkBlockCount ());
        ...
        while (true) {
            ...
            TransactionResult txResult = makeTransaction (mixin,
                                                          daemon,
                                                          ourInputs,
                                                          paymentID,
                                                          destinations,
     ...
     */
    std::tuple <Error, Crypto::Hash> sendTransactionBasic(std::string destination,
    ...
    sendTransactionAdvanced(std::vector <std::pair<std::string, uint64_t>> addressesAndAmounts,
    ...
    Error isTransactionPayloadTooBig(const CryptoNote::Transaction tx,
                                        ...
                                        const Crypto::Hash txHash)
    {
        ...
        for (const auto output : keyOutputs) {
             ...
             * Not our output
             */
            Crypto::underivePublicKey (derivation, outputIndex, output.key, spendKey);
    ...
    void storeSentTransaction(const Crypto::Hash hash,
                              const uint64_t fee,
                              const std::string paymentID,
    ...
    std::tuple <Error, Crypto::Hash> relayTransaction(const CryptoNote::Transaction tx,
                                                      const std::shared_ptr <Nigel> daemon)
    {
    ...
    setupDestinations(std::vector <std::pair<std::string, uint64_t>> addressesAndAmounts,
                      const uint64_t changeRequired,
                      const std::string changeAddress)
    {
        /*!
    ...
    std::tuple <Error, std::vector<CryptoNote::RandomOuts>> getRingParticipants(const uint64_t mixin,
                                                                                const std::shared_ptr <Nigel> daemon,
        ...
        for (auto fakeOut : fakeOuts) {
            ...
            if (fakeOut.outs.size () < mixin) {
                ...
                error
                    ...
                    << fakeOut.amount
                    << " ("
                    << Utilities::formatAmount (fakeOut.amount)
                    << "). Requested outputs: "
                    << requestedOuts
                    << ", found outputs: "
                    << fakeOut.outs.size ()
    ...
    prepareRingParticipants(std::vector <WalletTypes::TxInputAndOwner> sources,
                            const uint64_t mixin,
                            const std::shared_ptr <Nigel> daemon)
    {
        /*!
                                                                   ...
                                                                   const Crypto::SecretKey privateViewKey)
    {
         ...
         * view key
         */
        Crypto::generateKeyDerivation (input.realTransactionPublicKey, privateViewKey, derivation);
         ...
         */
        Crypto::derivePublicKey (derivation,
                                 input.realOutputTransactionIndex,
                                 input.ownerPublicSpendKey,
                                 tmpKeyPair.publicKey);

        /*!
         * Derive the secret key of the tmp key pair
         */
        Crypto::deriveSecretKey (derivation,
                                 input.realOutputTransactionIndex,
                                 input.ownerPrivateSpendKey,
    ...
    setupInputs(const std::vector <WalletTypes::ObscuredInput> inputsAndFakes,
                const Crypto::SecretKey privateViewKey)
    {
    ...
    setupOutputs(std::vector <WalletTypes::TransactionDestination> destinations)
    {
        ...
        Crypto::generateKeys (randomTxKey.publicKey, randomTxKey.secretKey);
        ...
        for (const auto destination : destinations) {
             ...
             */
            Crypto::derivePublicKey (derivation, outputIndex, destination.receiverPublicSpendKey, tmpPubKey);
    ...
    std::tuple <Error, CryptoNote::Transaction> generateRingSignatures(CryptoNote::Transaction tx,
                                                                       ...
                                                                       const std::vector <Crypto::SecretKey> tmpSecretKeys)
    {
        ...
        for (const auto input : inputsAndFakes) {
             ...
             */
            const auto[success, signatures] = Crypto::CryptoOps::generateRingSignatures (txPrefixHash,
                                                                                         boost::get<CryptoNote::KeyInput> (
        ...
        for (const auto input: inputsAndFakes) {
            ...
            if (!Crypto::CryptoOps::checkRingSignature (
                txPrefixHash,
    ...
    TransactionResult makeTransaction(const uint64_t mixin,
                                      const std::shared_ptr <Nigel> daemon,
                                      const std::vector <WalletTypes::TxInputAndOwner> ourInputs,
                                      const std::string paymentID,
                                      ...
                                      const uint64_t unlockTime)
    {
         ...
         */
        const auto[inputError, transactionInputs, tmpSecretKeys] = setupInputs (inputsAndFakes,
