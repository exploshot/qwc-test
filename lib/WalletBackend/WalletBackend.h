...
public:
     ...
     * or an error.
     */
    static std::tuple <Error, std::shared_ptr<WalletBackend>> importWalletFromSeed(const std::string mnemonicSeed,
    ...
    importWalletFromKeys(const Crypto::SecretKey privateSpendKey,
     ...
     */
    static std::tuple <Error, std::shared_ptr<WalletBackend>> importViewWallet(const Crypto::SecretKey privateViewKey,
    ...
    static Error saveWalletJSONToDisk(std::string walletJSON,
                                      std::string filename,
                                      std::string password);
     ...
     */
    std::tuple <Error, Crypto::Hash> sendTransactionBasic(const std::string destination,
    ...
    sendTransactionAdvanced(const std::vector <std::pair<std::string, uint64_t>> destinations,
...
private:
     ...
     */
    WalletBackend(const std::string filename,
                  const std::string password,
    ...
    static bool tryUpgradeWalletFormat(const std::string filename,
                                       const std::string password,
                                       const std::string daemonHost,
                                       const uint16_t daemonPort);
