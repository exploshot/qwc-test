 ...
 * Constructor
 */
BlockDownloader::BlockDownloader(const std::shared_ptr <Nigel> daemon,
...
bool BlockDownloader::shouldFetchMoreBlocks() const
{
    ...
    if (ramUsage + WalletConfig::maxBodyResponseSize < WalletConfig::blockStoreMemoryLimit) {
        ...
        stream
            << "Approximate ram usage of stored blocks: "
            << Utilities::prettyPrintBytes (ramUsage)
            << ", fetching more.";
...
bool BlockDownloader::downloadBlocks()
{
    ...
    if (blockCheckpoints.size () > 0) {
        ...
        stream
            << "First checkpoint: "
            << blockCheckpoints.front ()
            << "\nLast checkpoint: "
            << blockCheckpoints.back ();
...
void BlockDownloader::fromJSON(const JSONObject &j,
                               const uint64_t startHeight,
                               const uint64_t startTimestamp)
{
