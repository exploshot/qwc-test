...
namespace CryptoNote {
    ...
    NodeRpcProxy::NodeRpcProxy(const std::string &nodeHost,
    ...
    void NodeRpcProxy::updateBlockchainStatus()
    {
        ...
        if (!ec) {
            ...
            if (!parseHash256 (rsp.block_header.hash, blockHash)
                || !parseHash256 (rsp.block_header.prev_hash, prevBlockHash)) {
                return;
            }
    ...
    void NodeRpcProxy::getBlockHashesByTimestamps(uint64_t timestampBegin,
                                                  size_t secondsCount,
                                                  ...
                                                  const Callback &callback)
    {
        ...
        scheduleRequest (std::bind (&NodeRpcProxy::doGetBlockHashesByTimestamps,
                                    this,
                                    timestampBegin,
                                    secondsCount,
                                    std::ref (blockHashes)),
                                                       ...
                                                       const INode::Callback &callback)
    {
        ...
        scheduleRequest (std::bind (&NodeRpcProxy::doGetTransactionHashesByPaymentId,
                                    this,
                                    std::cref (paymentId),
                                    std::ref (transactionHashes)), callback);
    }

    std::error_code NodeRpcProxy::doGetBlockHashesByTimestamps(uint64_t timestampBegin,
                                                               size_t secondsCount,
                                                               std::vector <Crypto::Hash> &blockHashes)
    {
    ...
    void NodeRpcProxy::getRandomOutsByAmounts(std::vector <uint64_t> &&amounts,
                                              uint16_t outsCount,
                                              ...
                                              const Callback &callback)
    {
        ...
        scheduleRequest (std::bind (&NodeRpcProxy::doGetRandomOutsByAmounts,
                                    this,
                                    std::move (amounts),
                                    outsCount,
                                    std::ref (outs)),
                                                       ...
                                                       const Callback &callback)
    {
        ...
        scheduleRequest (std::bind (&NodeRpcProxy::doGetTransactionOutsGlobalIndices,
                                    this,
                                    transactionHash,
        ...
        const Callback &callback)
    {
        ...
        scheduleRequest (std::bind (&NodeRpcProxy::doGetGlobalIndexesForRange,
                                    this,
                                    startHeight,
                                    endHeight,
                                    std::ref (indexes)), callback);
    }
                                   ...
                                   const Callback &callback)
    {
        ...
        scheduleRequest (std::bind (&NodeRpcProxy::doQueryBlocksLite,
                                    this,
    ...
    void NodeRpcProxy::getWalletSyncData(std::vector <Crypto::Hash> &&knownBlockIds,
                                         ...
                                         const Callback &callback)
    {
        ...
        scheduleRequest (std::bind (&NodeRpcProxy::doGetWalletSyncData,
                                    this,
                                    std::move (knownBlockIds),
                                    startHeight,
                                    startTimestamp,
                                    std::ref (newBlocks)), callback);
    }

    void NodeRpcProxy::getPoolSymmetricDifference(std::vector <Crypto::Hash> &&knownPoolTxIds,
                                 ...
                                 const Callback &callback)
    {
        ...
        scheduleRequest (std::bind (&NodeRpcProxy::doGetBlocksByHeight,
                                    this,
                                    std::cref (blockHeights),
                                    std::ref (blocks)), callback);
    }
                                 ...
                                 const Callback &callback)
    {
        ...
        scheduleRequest (std::bind (&NodeRpcProxy::doGetBlocksByHash,
                                    this,
                                       ...
                                       const Callback &callback)
    {
        ...
        scheduleRequest (std::bind (&NodeRpcProxy::doGetTransactions,
                                    this,
                                    std::cref (transactionHashes),
                                    std::ref (transactions)), callback);
    }
    ...
    std::error_code NodeRpcProxy::doRelayTransaction(const CryptoNote::Transaction &transaction)
    {
        ...
        m_logger (TRACE)
            << "NodeRpcProxy::doRelayTransaction, tx hex "
            << req.tx_as_hex;
        return jsonCommand ("/sendrawtransaction", req, rsp);
    }

    std::error_code NodeRpcProxy::doGetRandomOutsByAmounts(std::vector <uint64_t> &amounts,
                                                           uint16_t outsCount,
                                                           std::vector <RandomOuts> &outs)
    {
                                                                    ...
                                                                    std::vector <uint32_t> &outsGlobalIndices)
    {
        ...
        m_logger (TRACE)
            << "Send get_o_indexes request, transaction "
            << req.txid;
        std::error_code ec = jsonCommand ("/get_o_indexes", req, rsp);
        if (!ec) {
            m_logger (TRACE)
                << "get_o_indexes complete";
    ...
    std::error_code NodeRpcProxy::doGetGlobalIndexesForRange(const uint64_t startHeight,
                                                             const uint64_t endHeight,
                                                             std::unordered_map <Crypto::Hash, std::vector<uint64_t>> &indexes)
    {
        ...
        m_logger (TRACE)
            << "Send get_global_indexes_for_range request";

        std::error_code ec = jsonCommand ("/get_global_indexes_for_range", req, rsp);

        if (!ec) {
            m_logger (TRACE)
                << "get_global_indexes_for_range complete";
        } else {
            m_logger (TRACE)
                << "get_global_indexes_for_range failed: "
                << ec
                << ", "
                << ec.message ();
        }
                                                          ...
                                                          std::unordered_set <Crypto::Hash> &transactionsUnknown)
    {
        ...
        m_logger (TRACE)
            << "Send get_transactions_status request";

        std::error_code ec = jsonCommand ("/get_transactions_status", req, rsp);

        if (!ec) {
            m_logger (TRACE)
                << "get_transactions_status complete";
        } else {
            m_logger (TRACE)
                << "get_transactions_status failed: "
                << ec
                << ", "
                << ec.message ();
        }
                                                    ...
                                                    uint32_t &startHeight)
    {
        ...
        m_logger (TRACE)
            << "Send queryblockslite request, timestamp "
            << req.timestamp;
        std::error_code ec = jsonCommand ("/queryblockslite", req, rsp);
        if (ec) {
            m_logger (TRACE)
                << "queryblockslite failed: "
                << ec
                << ", "
                << ec.message ();
            return ec;
        ...
        m_logger (TRACE)
            << "queryblockslite complete, startHeight "
            << rsp.startHeight
            << ", block count "
            << rsp.items.size ();
    ...
    std::error_code NodeRpcProxy::doGetWalletSyncData(const std::vector <Crypto::Hash> &knownBlockIds,
                                                      uint64_t startHeight,
                                                      uint64_t startTimestamp,
                                                      std::vector <WalletTypes::WalletBlockInfo> &newBlocks)
    {
        ...
        m_logger (TRACE)
            << "Send getwalletsyncdata request, start timestamp: "
            << req.startTimestamp
            << ", start height: "
            << req.startHeight;
        ...
        m_logger (TRACE)
            << "queryblockslite complete, block count "
            << rsp.items.size ();
                                                               ...
                                                               std::vector <Crypto::Hash> &deletedTxIds)
    {
        ...
        m_logger (TRACE)
            << "Send get_pool_changes_lite request, tailBlockId "
            << req.tailBlockId;
        std::error_code ec = jsonCommand ("/get_pool_changes_lite", req, rsp);

        if (ec) {
            m_logger (TRACE)
                << "get_pool_changes_lite failed: "
                << ec
                << ", "
                << ec.message ();
            return ec;
        ...
        m_logger (TRACE)
            << "get_pool_changes_lite complete, isTailBlockActual "
