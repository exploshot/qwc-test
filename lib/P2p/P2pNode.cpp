...
namespace CryptoNote {
          ...
          m_queueEvent (dispatcher)
    {
        ...
        for (auto &peer : cfg.getPeers ()) {
            m_peerlist.appendWithPeerWhite (peer);
        }
    }
    ...
    void P2pNode::acceptLoop()
    {
        while (!m_stopRequested) {
            try {
                ...
                auto ctx = new P2pContext (m_dispatcher,
                                           std::move (connection),
                                           true,
    ...
    void P2pNode::connectPeers()
    {
         ...
         */
        if (m_peerlist.getWhitePeersCount () == 0 && !m_cfg.getSeedNodes ().empty ()) {
    ...
    bool P2pNode::makeNewConnectionFromPeerlist(const Peerlist &peerlist)
    {
        ...
        for (size_t tryCount = 0; idxGen.generate (peerIndex) && tryCount < m_cfg.getPeerListGetTryCount ();
             ++tryCount) {
            PeerlistEntry peer;
    ...
    P2pNode::ContextPtr P2pNode::tryToConnectPeer(const NetworkAddress &address)
    {
        try {
            ...
            return ContextPtr (new P2pContext (m_dispatcher,
    ...
    {
        return m_peerlist.mergePeerlist (fixTimeDelta (peerlist, remoteTime));
    }
    ...
    std::list <PeerlistEntry> P2pNode::getLocalPeerList()
    {
        ...
        m_peerlist.getPeerlistHead (peerlist);
        return peerlist;
    }

    BasicNodeData P2pNode::getNodeData() const
    {
        BasicNodeData nodeData;
    ...
    void P2pNode::tryPing(P2pContext &ctx)
    {
        if (ctx.getPeerId () == m_myPeerId
            || !m_peerlist.isIpAllowed (ctx.getRemoteAddress ().ip)
            || ctx.getPeerPort () == 0) {
            return;
        }
        ...
        try {
            ...
            doWithTimeoutAndThrow (m_dispatcher, m_cfg.getHandshakeTimeout (), [&]
            {
                ...
                if (response.status == PING_OK_RESPONSE_STATUS_TEXT && response.peer_id == ctx.getPeerId ()) {
                    ...
                    m_peerlist.appendWithPeerWhite (entry);
                } else {
    ...
    void P2pNode::handleNodeData(const BasicNodeData &node, P2pContext &context)
    {
        ...
        if (!context.isIncoming ()) {
            m_peerlist.setPeerJustSeen (node.peer_id, context.getRemoteAddress ());
        }
    }
