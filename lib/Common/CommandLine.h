...
namespace CommandLine {
    ...
    template<typename T>
    boost::program_options::typed_value<T, char> *make_semantic(const ArgDescriptor<T, true> &/*arg*/)
    {
        return boost::program_options::value<T> ()->required ();
    }

    template<typename T>
    boost::program_options::typed_value<T, char> *make_semantic(const ArgDescriptor<T, false> &arg)
    {
    ...
    boost::program_options::typed_value<std::vector<T>, char> *make_semantic(
        const ArgDescriptor<std::vector<T>, false> &/*arg*/)
    {
    ...
    template<typename F>
    bool handleErrorHelper(const boost::program_options::options_description &desc, F parser)
    {
        try {
            return parser ();
            ...
            std::cerr
                << "Failed to parse arguments: unknown exception"
                << std::endl;
            std::cerr
                << desc
                << std::endl;
    ...
    template<typename T, bool required>
    T getArg(const boost::program_options::variables_map &vm, const ArgDescriptor<T, required> &arg)
    {
    ...
    extern const ArgDescriptor<bool> arg_help;

    extern const ArgDescriptor<bool> arg_version;

    extern const ArgDescriptor<std::string> arg_data_dir;
