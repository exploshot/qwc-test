                   ...
                   DataLength databitlen)
{
    ...
    /*!
        if the buffer contains data that has not yet been digested, 
        first add data to buffer until full 
    */
    if (ctx->bufPtr) {
                  ...
                  BitSequence *output)
{
    ...
    */
    if (BILB) {
        ctx->buffer[(int) ctx->bufPtr - 1] &= ((
                                                   1
                                                       << BILB
                                               ) - 1
        )
            << (8 - BILB);
        ctx->buffer[(int) ctx->bufPtr - 1] ^= 0x1
            << (7 - BILB);
        BILB = 0;
    } else {
