...
#define pre_aes() \
        ...
        _c = vld1q_u8(&hpState[j]); \
        _a = vld1q_u8((const uint8_t *)a); \

#define post_aes() \
        VARIANT2_SHUFFLE_ADD_NEON(hpState, j); \
        vst1q_u8((uint8_t *)c, _c); \
        vst1q_u8(&hpState[j], veorq_u8(_b, _c)); \
...
*/
static void aesExpandKey(const uint8_t *key, uint8_t *expandedKey)
{
...
#define mul(a, b, c)	cnMul128((const uint64_t *)a, (const uint64_t *)b, (uint64_t *)c)
STATIC void cnMul128(const uint64_t *a, const uint64_t *b, uint64_t *r)
{
    uint64_t lo, hi;
    __asm__("mul %0, %1, %2\n\t" : "=r"(lo) : "r"(a[0]), "r"(b[0]) );
    __asm__("umulh %0, %1, %2\n\t" : "=r"(hi) : "r"(a[0]), "r"(b[0]) );
...
#else /* ARM32 */
#define mul(a, b, c)	cnMul128((const uint32_t *)a, (const uint32_t *)b, (uint32_t *)c)
STATIC void cnMul128(const uint32_t *aa, const uint32_t *bb, uint32_t *r)
{
...
STATIC INLINE void sumHalfBlocks(uint8_t* a, const uint8_t* b)
{
...
STATIC INLINE void swapBlocks(uint8_t *a, uint8_t *b)
{
    uint64_t t[2];
    U64(t)[0] = U64(a)[0];
    U64(t)[1] = U64(a)[1];
