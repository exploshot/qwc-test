...
#define VARIANT2_INTEGER_MATH_SQRT_STEP_SSE2()                            \
    do {                                                                  \
        ...
        __m128d x = _mm_castsi128_pd(_mm_add_epi64(                       \
            _mm_cvtsi64_si128(sqrt_input >> 12), exp_double_bias));       \
        x = _mm_sqrt_sd(_mm_setzero_pd(), x);                             \
        sqrtResult = (uint64_t)(_mm_cvtsi128_si64(                        \
            _mm_sub_epi64(_mm_castpd_si128(x), exp_double_bias))) >> 19;  \
    } while(0)

#define VARIANT2_INTEGER_MATH_SQRT_STEP_FP64()                            \
    do {                                                                  \
        sqrtResult = sqrt(sqrt_input + 18446744073709551616.0) *          \
                     2.0 - 8589934592.0;                                  \
    } while(0)

#define VARIANT2_INTEGER_MATH_SQRT_STEP_REF() \
    sqrtResult = integerSquareRootV2(sqrt_input)
...
/*!
    ...
    1)  Write it as x = 1.NNNNNNNNNNNNNNNNNNNNNNNNNNNNNNNNNNNNNNNNNNNNNNNNNNNNNNNNNNNNNNNN000... 
        in binary (1 <= x < 2, all 64 bits of n are used)
    2)  Calculate sqrt(x) = 1.0RRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRR... (1 <= sqrt(x) < sqrt(2), 
        so it will always start with "1.0" in binary)
    3)  Take 32 bits that come after "1.0" and return them as a 32-bit unsigned integer, 
    ...
    The reference implementation as it is now uses only unsigned int64 arithmetic, 
    so it can't have undefined behavior
...
/*!
    ...
    "sqrt(2^64 + sqrt_input) * 2 - 2^33" and adds or subtracts 1 if needed
    It's hard to understand how it works, so here is a full calculation of formulas 
    used in VARIANT2_INTEGER_MATH_SQRT_FIXUP

    The following inequalities must hold for r if it's an integer part of 
    ...
    If this inequality doesn't hold, then we must decrement r: 
    IF "r2 + b^2/4 > sqrt_input" THEN r = r - 1

    b can be 0 or 1
    If b is 0 then we need to compare "r2 > sqrt_input"
    If b is 1 then b^2/4 = 0.25, so we need to compare "r2 + 0.25 > sqrt_input"
    Since both r2 and sqrt_input are integers, 
    we can safely replace it with "r2 + 1 > sqrt_input"
    -----------------------------------------------------------------------------------
    Both cases can be merged to a single expression "r2 + b > sqrt_input"
    -----------------------------------------------------------------------------------
    There will be no overflow when calculating "r2 + b", so it's safe to 
    compare with sqrt_input:
    r2 + b = s*(s+b) + r*2^32 + b
    The largest value s, b and r can have is 
    s = 1779033703, b = 1, r = 3558067407 when sqrt_input = 2^64 - 1
    ...
    If this inequality doesn't hold, then we must decrement r: 
    IF "r2 + 2^32 + (b+1)^2/4 <= sqrt_input - s" THEN r = r - 1

    b can be 0 or 1
    If b is 0 then we need to compare "r2 + 2^32 + 1/4 <= sqrt_input - s" 
    which is equal to "r2 + 2^32 < sqrt_input - s" because all numbers here are integers
    If b is 1 then (b+1)^2/4 = 1, so we need to compare "r2 + 2^32 + 1 <= sqrt_input - s" 
    which is also equal to "r2 + 2^32 < sqrt_input - s"
    -----------------------------------------------------------------------------------
    ...
    This inequality is true if sqrt_input > 1 and it's easy to check that s = 0 
    if sqrt_input is 0 or 1, so there will be no integer overflow
*/
