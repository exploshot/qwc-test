...
namespace CryptoNote {
    ...
    bool Currency::generateGenesisBlock()
    {
        ...
        */
        ...
        bool r = fromHex (genesisCoinbaseTxHex,
                          minerTxBlob) &&
                 fromBinaryArray (genesisBlockTemplate.baseTransaction,
                                  minerTxBlob);
    ...
    {
        return difficultyWindowByHeight (height) +
               difficultyLagByHeight (height);
    }
                                  ...
                                  uint32_t blockTarget) const
    {
        ...
        if (alreadyGeneratedCoins +
            CryptoNote::parameters::TAIL_EMISSION_REWARD >=
            m_moneySupply ||
            baseReward <
            CryptoNote::parameters::TAIL_EMISSION_REWARD) {
                                    ...
                                    size_t maxOuts/* = 1*/) const
    {
        ...
        int64_t emissionChange;
        if (!getBlockReward (blockMajorVersion,
                             medianSize,
                             currentBlockSize,
        ...
        decomposeAmountIntoDigits (blockReward, UINT64_C(0),
        ...
        for (size_t no = 0; no < outAmounts.size (); no++) {
            ...
            bool r = Crypto::generateKeyDerivation (minerAddress.viewPublicKey, txkey.secretKey, derivation);
            ...
            r = Crypto::derivePublicKey (derivation, no, minerAddress.spendPublicKey, outEphemeralPubKey);

            if (!(r)) {
                logger (ERROR, BRIGHT_RED)
                    << "while creating outs: failed to derivePublicKey("
                    << derivation
                    << ", "
                    << no
                    << ", "
    ...
    bool Currency::isFusionTransaction(const Transaction &transaction,
                                       size_t size,
                                       uint32_t height) const
    {
        ...
        return isFusionTransaction (getInputsAmounts (transaction),
                                    outputsAmounts,
                                    size,
                                    height);
    }
    ...
    bool Currency::isAmountApplicableInFusionTransactionInput(uint64_t amount,
                                                              uint64_t threshold,
                                                              uint32_t height) const
    {
        uint8_t ignore;
                                                              ...
                                                              uint32_t height) const
    {
        ...
        auto it = std::lower_bound (Constants::PRETTY_AMOUNTS.begin (),
        ...
        amountPowerOfTen = static_cast<uint8_t>(std::distance (Constants::PRETTY_AMOUNTS.begin (),
                                                               it) / 9);
    ...
    {
        return Utilities::getAccountAddressAsStr (m_publicAddressBase58Prefix,
                                                  account.getAccountKeys ().address);
    }
    ...
    bool Currency::parseAccountAddressString(const std::string &str,
                                             AccountPublicAddress &addr) const
    {
        uint64_t prefix;
    ...
    std::string Currency::formatAmount(uint64_t amount) const
    {
        ...
        */
        return s.substr (0,
                         s.size () -
                         (
                             m_numberOfDecimalPlaces -
                             m_numberOfDisplayDecimalPlaces
                         ));
    }
    ...
    uint64_t Currency::getNextDifficulty(uint32_t blockIndex,
    ...
    uint64_t Currency::nextDifficulty(uint32_t blockIndex,
                                      std::vector<uint64_t> timestamps,
                                      std::vector<uint64_t> cumulativeDifficulties) const
    {
        ...
        if (m_zawyDifficultyBlockIndex && m_zawyDifficultyBlockIndex <= blockIndex) {
            ...
            */
            ...
            std::vector<uint64_t> timestamps_tmp (timestamps_o.end () -
                                                  t_difficultyWindow,
                                                  timestamps_o.end ());
            std::vector<uint64_t> cumulativeDifficulties_tmp (cumulativeDifficulties_o.end () -
                                                              t_difficultyWindow,
    ...
    bool Currency::checkProofOfWorkV1(const CachedBlock &block,
                                      uint64_t currentDifficulty) const
    {
        if (BLOCK_MAJOR_VERSION_2 == block.getBlock ().majorVersion ||
            BLOCK_MAJOR_VERSION_3 == block.getBlock ().majorVersion) {
            return false;
        }

        return checkHash (block.getBlockLongHash (), currentDifficulty);
    }

    bool Currency::checkProofOfWorkV2(const CachedBlock &cachedBlock,
                                      uint64_t currentDifficulty) const
    {
        ...
        if (!checkHash (cachedBlock.getBlockLongHash (), currentDifficulty)) {
            return false;
        }
        ...
        Crypto::treeHashFromBranch (block.parentBlock.blockchainBranch.data (),
    ...
    bool Currency::checkProofOfWork(const CachedBlock &block,
                                    uint64_t currentDiffic) const
    {
    ...
    CurrencyBuilder::CurrencyBuilder(std::shared_ptr<Logging::ILogger> log)
        : m_currency (log)
    {
    ...
    Transaction CurrencyBuilder::generateGenesisTransaction()
    {
        ...
        /*!
            zero fee in genesis
        */
        m_currency.constructMinerTx (1, 0, 0, 0, 0, 0, ac, tx);
