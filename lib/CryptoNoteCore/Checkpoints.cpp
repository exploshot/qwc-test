...
namespace CryptoNote {
    ...
    bool Checkpoints::addCheckpoint(uint32_t index,
    ...
    bool Checkpoints::loadCheckpointsFromFile(const std::string &filename)
    {
        ...
        if (!file) {
            logger (ERROR, BRIGHT_RED)
                << "Could not load checkpoints file: "
                << filename;
    ...
    bool Checkpoints::checkBlock(uint32_t index,
                                 const Crypto::Hash &h,
