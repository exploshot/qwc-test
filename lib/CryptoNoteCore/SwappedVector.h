...
public:
    ...
    bool open(const std::string &itemFileName,
...
template<class T>
bool SwappedVector<T>::open(const std::string &itemFileName,
                            const std::string &indexFileName,
                            size_t poolSize)
{
    if (poolSize == 0) {
    ...
    if (m_itemsFile && m_indexesFile) {
        ...
        m_itemsFile.open (itemFileName,
                          ...
                          std::ios::out |
                          std::ios::binary);
        m_indexesFile.open (indexFileName,
                            std::ios::out |
                            std::ios::binary);
        uint64_t count = 0;
...
const T &SwappedVector<T>::operator[](uint64_t index)
{
    ...
    if (itemIter != m_items.end ()) {
        if (itemIter->second.cacheIter != --m_cache.end ()) {
            m_cache.splice (m_cache.end (),
                            m_cache,
                            itemIter->second.cacheIter);
        }
