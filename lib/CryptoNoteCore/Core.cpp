...
namespace CryptoNote {
    ...
    namespace {
        ...
        inline IBlockchainCache *findIndexInChain(IBlockchainCache *blockSegment,
                                                  ...
                                                  uint32_t blockIndex)
        {
            ...
            while (blockSegment != nullptr) {
                if (blockIndex >= blockSegment->getStartBlockIndex () &&
                    blockIndex < blockSegment->getStartBlockIndex () +
                                 blockSegment->getBlockCount ()) {
                    return blockSegment;
                }
        ...
        size_t getMaximumTransactionAllowedSize(size_t blockSizeMedian,
        ...
        int64_t getEmissionChange(const Currency &currency,
                                  ...
                                  uint64_t cumulativeFee)
        {
            ...
            if (!currency.getBlockReward (cachedBlock.getBlock ().majorVersion,
                                          blocksSizeMedian,
                                          cumulativeSize,
        ...
        uint32_t findCommonRoot(IMainChainStorage &storage, IBlockchainCache &rootSegment)
        {
            ...
            while (left != right) {
                ...
                if (getBlockHash (storage.getBlockByIndex (checkElement)) ==
                    rootSegment.getBlockHash (checkElement)) {
                    left = checkElement;
                } else {
    ...
    uint64_t Core::getBlockTimestampByIndex(uint32_t blockIndex) const
    {
        ...
        auto timestamps = chainsLeaves[0]->getLastTimestamps (1,
                                                              blockIndex,
                                                              addGenesisBlock);
    ...
    Block Core::getBlockByHash(const Crypto::Hash &blockHash) const
    {
        ...
        throwIfNotInitialized ();
        /*!
            TODO: should it be requested from the main chain?
        */
    ...
    std::vector<RawBlock> Core::getBlocks(uint32_t minIndex,
    ...
    void Core::getBlocks(const std::vector<Crypto::Hash> &blockHashes,
                         std::vector<RawBlock> &blocks,
    ...
    bool Core::queryBlocks(const std::vector<Crypto::Hash> &blockHashes,
                           uint64_t timestamp,
                           uint32_t &startIndex,
                           ...
                           std::vector<BlockFullInfo> &entries) const
    {
        ...
        try {
            ...
            size_t hashesPushed = pushBlockHashes (startIndex,
                                                   fullOffset,
            ...
            fillQueryBlockFullInfo (fullOffset,
                                    currentIndex,
    ...
    bool Core::queryBlocksLite(const std::vector<Crypto::Hash> &knownBlockHashes,
                               uint64_t timestamp,
                               uint32_t &startIndex,
                               uint32_t &currentIndex,
                               uint32_t &fullOffset,
                               std::vector<BlockShortInfo> &entries) const
    {
        ...
        try {
            ...
            */
            ...
            fillQueryBlockShortInfo (fullOffset,
                                     currentIndex,
                                     BLOCKS_SYNCHRONIZING_DEFAULT_COUNT,
                                     entries);
                                     ...
                                     std::unordered_set<Crypto::Hash> &transactionsUnknown) const
    {
        ...
        try {
            ...
            */
            ...
            logger (Logging::ERROR)
                << "Failed to get transactions status: "
                << e.what ();
    ...
    */
        ...
        try {
            ...
            */
            ...
            logger (Logging::DEBUGGING)
                ...
                << "\n* Known block hashes size: "
                << knownBlockHashes.size ()
                << "\n* Blocks requested: "
                << actualBlockCount
                << "\n* Start height: "
                << startHeight
                ...
                << "\n* First block height: "
                << firstBlockHeight
                << "\n* Last known block hash height: "
                << lastKnownBlockHashHeight
                << "\n* Start index: "
                << startIndex
                << "\n* Block difference: "
                << blockDifference
            ...
            */
            ...
            logger (Logging::ERROR)
                << "Failed to get wallet sync data: "
                << e.what ();
    ...
    void Core::getTransactions(const std::vector<Crypto::Hash> &transactionHashes,
                               ...
                               std::vector<Crypto::Hash> &missedHashes) const
    {
        ...
        */
        ...
        missedHashes.insert (missedHashes.end (),
                             leftTransactions.begin (),
                             leftTransactions.end ());
    }
    ...
    uint64_t Core::getBlockDifficulty(uint32_t blockIndex) const
    {
        throwIfNotInitialized ();
        IBlockchainCache *mainChain = chainsLeaves[0];
        auto difficulties = mainChain->getLastCumulativeDifficulties (2,
                                                                      blockIndex,
                                                                      addGenesisBlock);
    ...
    */
    uint64_t Core::getDifficultyForNextBlock() const
    {
        ...
        size_t blocksCount = std::min (static_cast<size_t>(topBlockIndex),
                                       currency.difficultyBlocksCountByHeight (
                                           topBlockIndex
                                       )
        );
        ...
        return currency.getNextDifficulty (topBlockIndex,
                                           timestamps,
                                           difficulties);
    }
    ...
    std::error_code Core::addBlock(const CachedBlock &cachedBlock,
                                   RawBlock &&rawBlock)
    {
        throwIfNotInitialized ();
        ...
        if (!extractTransactions (rawBlock.transactions, transactions, cumulativeSize)) {
            logger (Logging::DEBUGGING)
                << "Couldn't deserialize raw block transactions in block "
                << blockStr;
        ...
        */
        if (blockIndex >= CryptoNote::parameters::BLOCK_BLOB_SHUFFLE_CHECK_HEIGHT) {
            ...
            */
            if (!Utilities::is_unique (blockTemplate.transactionHashes.begin (),
                                       blockTemplate.transactionHashes.end ())) {
            ...
            */
            if (!Utilities::is_unique (transactionHashes.begin (),
            ...
            */
            for (const auto &transaction : transactionHashes) {
                const auto search = std::find (blockTemplate.transactionHashes.begin (),
                                               blockTemplate.transactionHashes.end (),
                                               transaction);
        ...
        */
        ...
        if (mixinChangeWindow > CryptoNote::parameters::CRYPTONOTE_MINED_MONEY_UNLOCK_WINDOW) {
            mixinChangeWindow = mixinChangeWindow -
        ...
        for (const auto &transaction : transactions) {
            uint64_t fee = 0;
            auto transactionValidationResult = validateTransaction (transaction,
                                                                    validatorState,
                                                                    cache,
                                                                    fee,
                                                                    previousBlockIndex);
        ...
        */
        if (checkpoints.isInCheckpointZone (cachedBlock.getBlockIndex ())) {
            if (!checkpoints.checkBlock (cachedBlock.getBlockIndex (), cachedBlock.getBlockHash ())) {
                logger (Logging::WARNING)
                    << "Checkpoint block hash mismatch for block "
                    << blockStr;
            ...
            if (cache->getChildCount () == 0) {
                ...
                */
                if (cache == chainsLeaves[0]) {
                    ...
                    notifyObservers (makeDelTransactionMessage (std::move (hashes),
                                                                Messages::DeleteTransaction::Reason::InBlock));
                } else {
                    cache->pushBlock (cachedBlock,
                                      transactions,
                                      validatorState,
                ...
                /*!
                    add block on top of segment which is not leaf! the case when we got more than 
                    one alternative block on the same height
                */
                ...
                newlyForkedChainPtr->pushBlock (cachedBlock,
                                                transactions,
                                                validatorState,
                                                cumulativeBlockSize,
                                                emissionChange,
    ...
    void Core::actualizePoolTransactions()
    {
        ...
        for (auto &hash : hashes) {
            ...
            if (!addTransactionToPool (std::move (tx))) {
                notifyObservers (makeDelTransactionMessage ({hash},
    ...
    void Core::actualizePoolTransactionsLite(const TransactionValidatorState &validatorState)
    {
        ...
        for (auto &hash : hashes) {
            ...
            if (hasIntersections (validatorState, txState) ||
                ...
                getMaximumTransactionAllowedSize (blockMedianSize, currency) ||
    ...
    void Core::notifyOnSuccess(error::AddBlockErrorCode opResult,
                               uint32_t previousBlockIndex,
                               const CachedBlock &cachedBlock,
    ...
    std::error_code Core::submitBlock(BinaryArray &&rawBlockTemplate)
    {
        ...
        if (!result) {
            logger (Logging::WARNING)
                << "Couldn't deserialize block template";
    ...
    bool Core::getRandomOutputs(uint64_t amount,
                                uint16_t count,
                                ...
                                std::vector<Crypto::PublicKey> &publicKeys) const
    {
        ...
        if (upperBlockLimit < currency.minedMoneyUnlockWindow ()) {
            logger (Logging::DEBUGGING)
                << "Blockchain height is less than mined unlock window";
        ...
        if (globalIndexes.empty ()) {
            logger (Logging::ERROR)
                << "Failed to get any matching outputs for amount "
                << amount
                << " ("
        ...
        switch (chainsLeaves[0]->extractKeyOutputKeys (amount,
                                                       getTopBlockIndex (),
                                                       {
                                                           globalIndexes.data (),
                                                           globalIndexes.size ()
    ...
    bool Core::getGlobalIndexesForRange(const uint64_t startHeight,
                                        const uint64_t endHeight,
                                        std::unordered_map<Crypto::Hash,
                                                           std::vector<uint64_t>> &indexes) const
    {
        throwIfNotInitialized ();
    ...
    bool Core::addTransactionToPool(const BinaryArray &transactionBinaryArray)
    {
        ...
        if (!fromBinaryArray<Transaction> (transaction, transactionBinaryArray)) {
            logger (Logging::WARNING)
                << "Couldn't add transaction to pool due to deserialization error";
            return false;
        }
                                         ...
                                         TransactionValidatorState &validatorState)
    {
        ...
        if (auto validationResult = validateTransaction (cachedTransaction,
                                                         validatorState,
                                                         chainsLeaves[0],
                                                         fee,
                                                         getTopBlockIndex ())) {
        ...
        bool isFusion = fee == 0 && currency.isFusionTransaction (cachedTransaction.getTransaction (),
                                                                  cachedTransaction.getTransactionBinaryArray ()
                                                                                   .size (),
                                                                  getTopBlockIndex ());
    ...
    bool Core::getPoolChanges(const Crypto::Hash &lastBlockHash,
                              ...
                              std::vector<Crypto::Hash> &deletedTransactions) const
    {
        ...
        getTransactionPoolDifference (knownHashes,
                                      newTransactions,
                                      deletedTransactions);
    ...
    bool Core::getPoolChangesLite(const Crypto::Hash &lastBlockHash,
                                  const std::vector<Crypto::Hash> &knownHashes,
                                ...
                                uint32_t &height) const
    {
        ...
        if (b.majorVersion == BLOCK_MAJOR_VERSION_1) {
            b.minorVersion = currency.upgradeHeight (BLOCK_MAJOR_VERSION_2) ==
                             IUpgradeDetector::UNDEF_HEIGHT ? BLOCK_MINOR_VERSION_1
                                                            : BLOCK_MINOR_VERSION_0;
        } else if (b.majorVersion == BLOCK_MAJOR_VERSION_2 ||
                   b.majorVersion == BLOCK_MAJOR_VERSION_3) {
            if (currency.upgradeHeight (BLOCK_MAJOR_VERSION_3) ==
            ...
            b.minorVersion = currency.upgradeHeight (BLOCK_MAJOR_VERSION_4) ==
                             IUpgradeDetector::UNDEF_HEIGHT
                             ? BLOCK_MINOR_VERSION_1
                             : BLOCK_MINOR_VERSION_0;
        } else if (b.majorVersion >= BLOCK_MAJOR_VERSION_5) {
            b.minorVersion = currency.upgradeHeight (BLOCK_MAJOR_VERSION_5) ==
                             IUpgradeDetector::UNDEF_HEIGHT
                             ? BLOCK_MINOR_VERSION_1
                             : BLOCK_MINOR_VERSION_0;
        } else if (b.majorVersion >= BLOCK_MAJOR_VERSION_6) {
            b.minorVersion = currency.upgradeHeight (BLOCK_MAJOR_VERSION_6) ==
                             IUpgradeDetector::UNDEF_HEIGHT
                             ? BLOCK_MINOR_VERSION_1
                             : BLOCK_MINOR_VERSION_0;
        }
        ...
        */
        bool r = currency.constructMinerTx (b.majorVersion,
                                            height,
                                            medianSize,
                                            alreadyGeneratedCoins,
                                            transactionsSize,
        ...
        for (size_t tryCount = 0; tryCount < TRIES_COUNT; ++tryCount) {
            r = currency.constructMinerTx (b.majorVersion,
                                           height, medianSize,
                                           alreadyGeneratedCoins,
                                           cumulativeSize,
                                           fee,
    ...
    CoreStatistics Core::getCoreStatistics() const
    {
        ...
        */
        ...
        std::fill (reinterpret_cast<uint8_t *>(&result),
    ...
    size_t Core::getAlternativeBlockCount() const
    {
        ...
        return std::accumulate (chainsStorage.begin (),
                                chainsStorage.end (),
                                size_t (0),
                                [&](size_t sum,
                                    const Ptr &ptr)
    ...
    std::error_code Core::validateTransaction(const CachedTransaction &cachedTransaction,
        ...
        for (const auto &input : transaction.inputs) {
            if (input.type () == typeid (KeyInput)) {
                ...
                if (!checkpoints.isInCheckpointZone (blockIndex + 1)) {
                    ...
                    auto result = cache->extractKeyOutputKeys (in.amount,
                                                               blockIndex,
                                                               {
                                                                   globalIndexes.data (),
                                                                   globalIndexes.size ()
                    ...
                    if (blockIndex >=
                        CryptoNote::parameters::TRANSACTION_SIGNATURE_COUNT_VALIDATION_HEIGHT &&
                        outputKeys.size () !=
                        cachedTransaction.getTransaction ().signatures[inputIndex].size ()) {
                        return error::TransactionValidationError::INPUT_INVALID_SIGNATURES_COUNT;
                    }

                    if (!Crypto::CryptoOps::checkRingSignature (cachedTransaction.getTransactionPrefixHash (),
                                                                in.keyImage,
                                                                outputKeys,
    ...
    std::error_code Core::validateSemantic(const Transaction &transaction,
                                           uint64_t &fee,
                                           uint32_t blockIndex)
    {
        ...
        */
        ...
        for (const auto &output : transaction.outputs) {
            ...
            if (output.target.type () == typeid (KeyOutput)) {
                if (!checkKey (boost::get<KeyOutput> (output.target).key)) {
        ...
        for (const auto &input : transaction.inputs) {
            ...
            if (input.type () == typeid (KeyInput)) {
                ...
                */
                ...
                if (std::find (++std::begin (in.outputIndexes),
                               std::end (in.outputIndexes), 0) !=
    ...
    std::vector<Crypto::Hash> CryptoNote::Core::getBlockHashes(uint32_t startBlockIndex,
                                                               uint32_t maxCount) const
    {
        return chainsLeaves[0]->getBlockHashes (startBlockIndex, maxCount);
    }

    std::error_code Core::validateBlock(const CachedBlock &cachedBlock,
                                        IBlockchainCache *cache,
                                        uint64_t &minerReward)
    {
        ...
        if (block.majorVersion >= BLOCK_MAJOR_VERSION_1) {
            if (block.majorVersion == BLOCK_MAJOR_VERSION_2 &&
                block.parentBlock.majorVersion > BLOCK_MAJOR_VERSION_1) {
                logger (Logging::ERROR, Logging::BRIGHT_RED)
                    << "Parent block of block "
        ...
        if (block.timestamp > getAdjustedTime () +
                              currency.blockFutureTimeLimit (previousBlockIndex + 1)) {
    ...
    void Core::load()
    {
        ...
        logger (Logging::DEBUGGING)
            << "Blockchain storage blocks count: "
            << storageBlocksCount
            << ", DB blocks count: "
            << dbBlocksCount;

        /*!
            we assume the storage has at least genesis block
        */
        assert(storageBlocksCount != 0);

        if (storageBlocksCount > dbBlocksCount) {
            logger (Logging::INFO)
                << "Importing blocks from blockchain storage";
            ...
            logger (Logging::INFO)
                << "DB has more blocks than blockchain storage, cutting from block index: "
                << cutFrom;
            ...
            logger (Logging::INFO)
                << "Blockchain storage and root segment are on different chains. "
                << "Cutting root segment to common block index "
                << findCommonRoot (*mainChainStorage, *chainsLeaves[0])
                << " and reimporting blocks";

            importBlocksFromStorage ();
        } else {
            logger (Logging::DEBUGGING)
                << "Blockchain storage and root segment are on the same height and chain";
        }
    ...
    void Core::importBlocksFromStorage()
    {
        ...
        for (uint32_t i = commonIndex + 1; i < blockCount; ++i) {
            ...
            if (blockTemplate.previousBlockHash != previousBlockHash) {
                logger (Logging::ERROR)
                    << "Local blockchain corruption detected. "
                    << std::endl
                    << "Block with index "
                    << i
                    << " and hash "
                    << cachedBlock.getBlockHash ()
                    << " has previous block hash "
            ...
            uint64_t cumulativeFee = std::accumulate (transactions.begin (),
                                                      transactions.end (),
                                                      UINT64_C(0),
                                                      [](uint64_t fee,
            ...
            int64_t emissionChange = getEmissionChange (currency,
                                                        *chainsLeaves[0],
                                                        i - 1,
                                                        cachedBlock,
                                                        cumulativeSize,
                                                        cumulativeFee);
            chainsLeaves[0]->pushBlock (cachedBlock,
                                        transactions,
                                        spentOutputs,
                                        cumulativeSize,
                                        emissionChange,
    ...
    void Core::cutSegment(IBlockchainCache &segment, uint32_t startIndex)
    {
        ...
        logger (Logging::INFO)
            << "Cutting root segment from index "
            << startIndex;
    ...
    IBlockchainCache *Core::findAlternativeSegmentContainingBlock(const Crypto::Hash &blockHash) const
    {
        ...
        std::find_if (++chainsLeaves.begin (),
                      chainsLeaves.end (),
    ...
    */
    ...
    {
        assert(blockIndex >= segment->getStartBlockIndex () &&
               blockIndex <= segment->getTopBlockIndex ());

        return segment->getBlockByIndex (blockIndex);
    }

    //TODO: decompose these three methods
    size_t Core::pushBlockHashes(uint32_t startIndex,
                                 uint32_t fullOffset,
    ...
    void Core::fillQueryBlockFullInfo(uint32_t fullOffset,
                                      uint32_t currentIndex,
                                      size_t maxItemsCount,
                                      std::vector<BlockFullInfo> &entries) const
    {
        ...
        uint32_t fullBlocksCount = static_cast<uint32_t>(std::min (static_cast<uint32_t>(maxItemsCount),
                                                                   currentIndex - fullOffset));
        entries.reserve (entries.size () + fullBlocksCount);

        for (uint32_t blockIndex = fullOffset;
             blockIndex < fullOffset + fullBlocksCount;
             ++blockIndex) {
    ...
    void Core::fillQueryBlockShortInfo(uint32_t fullOffset,
                                       uint32_t currentIndex,
                                       size_t maxItemsCount,
                                       std::vector<BlockShortInfo> &entries) const
    {
        ...
        for (uint32_t blockIndex = fullOffset;
            ...
            for (auto &rawTransaction : rawBlock.transactions) {
                ...
                */
                prefixInfo.txHash = getBinaryArrayHash (rawTransaction);
    ...
    void Core::fillQueryBlockDetails(uint32_t fullOffset,
                                     uint32_t currentIndex,
                                     size_t maxItemsCount,
                                     std::vector<BlockDetails> &entries) const
    {
    ...
    */
                                 ...
                                 uint64_t &fee) const
    {
        ...
        */
        const auto addTransactionToBlockTemplate = [this,
                                                    &spentInputsChecker,
                                                    maxTotalSize,
                                                    height,
                                                    &transactionsSize,
                                                    &fee,
                                                    &block](const CachedTransaction &transaction)
        {
            /*!
    ...
    void Core::mergeMainChainSegments()
    {
        ...
        auto rootIt = std::find_if (chainsStorage.begin (),
                                    chainsStorage.end (),
    ...
    {
        assert(segment->getStartBlockIndex () == acceptingSegment->getStartBlockIndex () +
                                                 acceptingSegment->getBlockCount ());

        auto startIndex = segment->getStartBlockIndex ();
        auto blockCount = segment->getBlockCount ();
        for (auto blockIndex = startIndex;
             blockIndex < startIndex + blockCount;
             ++blockIndex) {
            ...
            if (!Utils::restoreCachedTransactions (info.rawBlock.transactions, transactions)) {
                logger (Logging::WARNING)
                    << "mergeSegments error: Couldn't deserialize transactions";
                throw std::runtime_error ("Couldn't deserialize transactions");
            }

            acceptingSegment->pushBlock (CachedBlock (block),
                                         transactions,
                                         info.validatorState,
                                         info.blockSize,
                                         info.generatedCoins,
    ...
    BlockDetails Core::getBlockDetails(const Crypto::Hash &blockHash) const
    {
        ...
        int64_t emissionChange = 0;
        bool result = currency.getBlockReward (blockDetails.majorVersion,
        ...
        result = currency.getBlockReward (blockDetails.majorVersion,
                                          blockDetails.sizeMedian,
        ...
        blockDetails.transactions.push_back (getTransactionDetails (cachedBaseTx.getTransactionHash (),
                                                                    segment, false));
    ...
    TransactionDetails Core::getTransactionDetails(const Crypto::Hash &transactionHash,
                                                   IBlockchainCache *segment,
    ...
    std::vector<Crypto::Hash> Core::getBlockHashesByTimestamps(uint64_t timestampBegin,
                                                               size_t secondsCount) const
    {
        throwIfNotInitialized ();

        logger (Logging::DEBUGGING)
            << "getBlockHashesByTimestamps request with timestamp "
            << timestampBegin
            << " and seconds count "
            << secondsCount;
        ...
        if (timestampBegin + static_cast<uint64_t>(secondsCount) < timestampBegin) {
            logger (Logging::WARNING)
                << "Timestamp overflow occured. Timestamp begin: "
                << timestampBegin
                << ", timestamp end: "
                << (timestampBegin + static_cast<uint64_t>(secondsCount));
    ...
    std::vector<Crypto::Hash> Core::getTransactionHashesByPaymentId(const Hash &paymentId) const
    {
        ...
        logger (Logging::DEBUGGING)
            << "getTransactionHashesByPaymentId request with paymentId "
            << paymentId;
    ...
    void Core::transactionPoolCleaningProcedure()
    {
        ...
        try {
            ...
            logger (Logging::DEBUGGING)
                << "transactionPoolCleaningProcedure has been interrupted";
        } catch (std::exception &e) {
            logger (Logging::ERROR)
                << "Error occurred while cleaning transactions pool: "
                << e.what ();
        }
    }
    ...
    uint64_t Core::getCurrentBlockchainHeight() const
    {
        /*!
