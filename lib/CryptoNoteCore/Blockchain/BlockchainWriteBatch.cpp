                                           ...
                                           const std::unordered_set<Crypto::KeyImage> &spentKeyImages)
{
    ...
    rawDataToInsert.emplace_back (DB::serialize (DB::BLOCK_INDEX_TO_KEY_IMAGE_PREFIX,
                                                 blockIndex,
                                                 spentKeyImages));
    for (const Crypto::KeyImage &keyImage : spentKeyImages) {
        rawDataToInsert.emplace_back (DB::serialize (DB::KEY_IMAGE_TO_BLOCK_INDEX_PREFIX,
                                                     keyImage,
                                                     blockIndex));
    }
...
{
    rawDataToInsert.emplace_back (DB::serialize (DB::TRANSACTION_HASH_TO_TRANSACTION_INFO_PREFIX,
                                      ...
                                      uint32_t totalTxsCountForPaymentId)
{
    ...
    rawDataToInsert.emplace_back (DB::serialize (DB::PAYMENT_ID_TO_TX_HASH_PREFIX,
                                                 std::make_pair (paymentId,
                                                                 totalTxsCountForPaymentId - 1),
                                                 transactionHash));
...
{
    rawDataToInsert.emplace_back (DB::serialize (DB::BLOCK_INDEX_TO_BLOCK_INFO_PREFIX,
                                                 blockIndex,
                                                 block));
    rawDataToInsert.emplace_back (DB::serialize (DB::BLOCK_INDEX_TO_TX_HASHES_PREFIX,
                                                 blockIndex,
                                                 blockTxs));
    rawDataToInsert.emplace_back (DB::serialize (DB::BLOCK_HASH_TO_BLOCK_INDEX_PREFIX,
                                                 block.blockHash,
                                                 blockIndex));
    rawDataToInsert.emplace_back (DB::serialize (DB::BLOCK_INDEX_TO_BLOCK_HASH_PREFIX,
                                                   ...
                                                   uint32_t totalOutputsCountForAmount)
{
    ...
    rawDataToInsert.emplace_back (DB::serialize (DB::KEY_OUTPUT_AMOUNT_PREFIX,
                                                 amount,
...
BlockchainWriteBatch &
BlockchainWriteBatch::insertRawBlock(uint32_t blockIndex,
                                     const RawBlock &block)
{
    rawDataToInsert.emplace_back (DB::serialize (DB::BLOCK_INDEX_TO_RAW_BLOCK_PREFIX,
                                                 blockIndex,
                                                 block));
...
BlockchainWriteBatch &
BlockchainWriteBatch::insertClosestTimestampBlockIndex(uint64_t timestamp,
                                                       uint32_t blockIndex)
{
    rawDataToInsert.emplace_back (DB::serialize (DB::CLOSEST_TIMESTAMP_BLOCK_INDEX_PREFIX,
                                                 timestamp,
                                                 blockIndex));
...
BlockchainWriteBatch &
BlockchainWriteBatch::insertTimestamp(uint64_t timestamp,
                                      const std::vector<Crypto::Hash> &blockHashes)
{
    rawDataToInsert.emplace_back (DB::serialize (DB::TIMESTAMP_TO_BLOCKHASHES_PREFIX,
                                                 timestamp,
                                                 blockHashes));
...
{
    rawDataToInsert.emplace_back (DB::serialize (DB::KEY_OUTPUT_KEY_PREFIX,
                                                 std::make_pair (amount,
                                                                 globalIndex),
                                                 outputInfo));
                                           ...
                                           const std::vector<Crypto::KeyImage> &spentKeyImages)
{
    ...
    rawKeysToRemove.emplace_back (DB::serializeKey (DB::BLOCK_INDEX_TO_KEY_IMAGE_PREFIX,
                                                    blockIndex));

    for (const Crypto::KeyImage &keyImage : spentKeyImages) {
        rawKeysToRemove.emplace_back (DB::serializeKey (DB::KEY_IMAGE_TO_BLOCK_INDEX_PREFIX,
                                                        keyImage));
    }
...
BlockchainWriteBatch &
BlockchainWriteBatch::removeCachedTransaction(const Crypto::Hash &transactionHash,
                                              uint64_t totalTxsCount)
{
    rawKeysToRemove.emplace_back (DB::serializeKey (DB::TRANSACTION_HASH_TO_TRANSACTION_INFO_PREFIX,
                                                    transactionHash));
    rawDataToInsert.emplace_back (DB::serialize (DB::TRANSACTION_HASH_TO_TRANSACTION_INFO_PREFIX,
                                                 DB::TRANSACTIONS_COUNT_KEY,
                                                 totalTxsCount));
...
BlockchainWriteBatch &
BlockchainWriteBatch::removePaymentId(const Crypto::Hash paymentId,
                                      uint32_t totalTxsCountForPaymentId)
{
    rawDataToInsert.emplace_back (DB::serialize (DB::PAYMENT_ID_TO_TX_HASH_PREFIX,
                                                 paymentId,
                                                 totalTxsCountForPaymentId));
    rawKeysToRemove.emplace_back (DB::serializeKey (DB::PAYMENT_ID_TO_TX_HASH_PREFIX,
                                                    std::make_pair (paymentId,
                                                                    totalTxsCountForPaymentId)));
...
BlockchainWriteBatch &
BlockchainWriteBatch::removeCachedBlock(const Crypto::Hash &blockHash,
                                        uint32_t blockIndex)
{
    rawKeysToRemove.emplace_back (DB::serializeKey (DB::BLOCK_INDEX_TO_BLOCK_INFO_PREFIX,
                                                    blockIndex));
    rawKeysToRemove.emplace_back (DB::serializeKey (DB::BLOCK_INDEX_TO_TX_HASHES_PREFIX,
                                                    blockIndex));
    rawKeysToRemove.emplace_back (DB::serializeKey (DB::BLOCK_HASH_TO_BLOCK_INDEX_PREFIX,
                                                    blockHash));
    rawDataToInsert.emplace_back (DB::serialize (DB::BLOCK_INDEX_TO_BLOCK_HASH_PREFIX,
                                                 DB::LAST_BLOCK_INDEX_KEY,
                                                 blockIndex - 1));
                                                   ...
                                                   uint32_t totalOutputsCountForAmount)
{
    ...
    for (uint32_t i = 0; i < outputsToRemoveCount; ++i) {
        rawKeysToRemove.emplace_back (DB::serializeKey (DB::KEY_OUTPUT_AMOUNT_PREFIX,
                                                        std::make_pair (amount,
                                                                        totalOutputsCountForAmount + i)));
    }
...
{
    rawKeysToRemove.emplace_back (DB::serializeKey (DB::BLOCK_INDEX_TO_RAW_BLOCK_PREFIX,
                                                    blockIndex));
...
{
    rawKeysToRemove.emplace_back (DB::serializeKey (DB::CLOSEST_TIMESTAMP_BLOCK_INDEX_PREFIX,
                                                    timestamp));
...
{
    rawKeysToRemove.emplace_back (DB::serializeKey (DB::TIMESTAMP_TO_BLOCKHASHES_PREFIX,
                                                    timestamp));
...
BlockchainWriteBatch::removeKeyOutputInfo(IBlockchainCache::Amount amount,
                                          IBlockchainCache::GlobalOutputIndex globalIndex)
{
    rawKeysToRemove.emplace_back (DB::serializeKey (DB::KEY_OUTPUT_KEY_PREFIX,
                                                    std::make_pair (amount,
                                                                    globalIndex)));
...
BlockchainWriteBatch::extractRawDataToInsert()
{
