...
{
    state.spentKeyImagesByBlock.emplace (blockIndex,
                                         std::vector<Crypto::KeyImage> ());
    return *this;
}
...
{
    state.cachedTransactions.emplace (txHash,
...
BlockchainReadBatch::requestCachedTransactions(const std::vector<Crypto::Hash> &transactions)
{
    for (const auto hash : transactions) {
        state.cachedTransactions.emplace (hash,
                                          ExtendedTransactionInfo ());
    }
...
{
    state.transactionHashesByBlocks.emplace (blockIndex,
...
{
    state.cachedBlocks.emplace (blockIndex,
                                CachedBlockInfo ());
    return *this;
}
...
{
    state.keyOutputGlobalIndexesForAmounts.emplace (std::make_pair (amount,
                                                                    outputIndexWithinAmout),
                                                    PackedOutIndex ());
    return *this;
}
...
BlockchainReadBatch &BlockchainReadBatch::requestRawBlocks(uint64_t startHeight,
                                                           uint64_t endHeight)
{
...
{
    state.transactionHashesByPaymentIds.emplace (std::make_pair (paymentId,
                                                                 transactionIndexWithinPaymentId),
                                                 Constants::NULL_HASH);
    return *this;
}
...
{
    state.keyOutputKeys.emplace (std::make_pair (amount,
                                                 globalIndex),
                                 KeyOutputInfo{});
    return *this;
}
...
std::vector<std::string> BlockchainReadBatch::getRawKeys() const
{
    ...
    DB::serializeKeys (rawKeys,
                       DB::BLOCK_INDEX_TO_KEY_IMAGE_PREFIX,
                       state.spentKeyImagesByBlock);
    DB::serializeKeys (rawKeys,
                       DB::KEY_IMAGE_TO_BLOCK_INDEX_PREFIX,
                       state.blockIndexesBySpentKeyImages);
    DB::serializeKeys (rawKeys,
                       DB::TRANSACTION_HASH_TO_TRANSACTION_INFO_PREFIX,
                       state.cachedTransactions);
    DB::serializeKeys (rawKeys,
                       DB::BLOCK_INDEX_TO_TX_HASHES_PREFIX,
                       state.transactionHashesByBlocks);
    DB::serializeKeys (rawKeys,
                       DB::BLOCK_INDEX_TO_BLOCK_INFO_PREFIX,
                       state.cachedBlocks);
    DB::serializeKeys (rawKeys,
                       DB::BLOCK_HASH_TO_BLOCK_INDEX_PREFIX,
                       state.blockIndexesByBlockHashes);
    DB::serializeKeys (rawKeys,
                       DB::KEY_OUTPUT_AMOUNT_PREFIX,
                       state.keyOutputGlobalIndexesCountForAmounts);
    DB::serializeKeys (rawKeys,
                       DB::KEY_OUTPUT_AMOUNT_PREFIX,
                       state.keyOutputGlobalIndexesForAmounts);
    DB::serializeKeys (rawKeys,
                       DB::BLOCK_INDEX_TO_RAW_BLOCK_PREFIX,
                       state.rawBlocks);
    DB::serializeKeys (rawKeys,
                       DB::CLOSEST_TIMESTAMP_BLOCK_INDEX_PREFIX,
                       state.closestTimestampBlockIndex);
    DB::serializeKeys (rawKeys,
                       DB::KEY_OUTPUT_AMOUNTS_COUNT_PREFIX,
                       state.keyOutputAmounts);
    DB::serializeKeys (rawKeys,
                       DB::PAYMENT_ID_TO_TX_HASH_PREFIX,
                       state.transactionCountsByPaymentIds);
    DB::serializeKeys (rawKeys,
                       DB::PAYMENT_ID_TO_TX_HASH_PREFIX,
                       state.transactionHashesByPaymentIds);
    DB::serializeKeys (rawKeys,
                       DB::TIMESTAMP_TO_BLOCKHASHES_PREFIX,
                       state.blockHashesByTimestamp);
    DB::serializeKeys (rawKeys,
                       DB::KEY_OUTPUT_KEY_PREFIX,
                       state.keyOutputKeys);

    if (state.lastBlockIndex.second) {
        rawKeys.emplace_back (DB::serializeKey (DB::BLOCK_INDEX_TO_BLOCK_HASH_PREFIX,
                                                DB::LAST_BLOCK_INDEX_KEY));
    }

    if (state.keyOutputAmountsCount.second) {
        rawKeys.emplace_back (DB::serializeKey (DB::KEY_OUTPUT_AMOUNTS_COUNT_PREFIX,
                                                DB::KEY_OUTPUT_AMOUNTS_COUNT_KEY));
    }

    if (state.transactionsCount.second) {
        rawKeys.emplace_back (DB::serializeKey (DB::TRANSACTION_HASH_TO_TRANSACTION_INFO_PREFIX,
                                                DB::TRANSACTIONS_COUNT_KEY));
    }
...
BlockchainReadResult::BlockchainReadResult(BlockchainReadState _state)
...
const std::unordered_map<Crypto::KeyImage, uint32_t> &
BlockchainReadResult::getBlockIndexesBySpentKeyImages() const
{
    return state.blockIndexesBySpentKeyImages;
}

const std::unordered_map<Crypto::Hash, ExtendedTransactionInfo> &
...
const std::unordered_map<uint32_t, CachedBlockInfo> &
BlockchainReadResult::getCachedBlocks() const
{
    return state.cachedBlocks;
}

const std::unordered_map<Crypto::Hash, uint32_t> &
...
const std::unordered_map<std::pair<IBlockchainCache::Amount,
                                   uint32_t>, PackedOutIndex> &
BlockchainReadResult::getKeyOutputGlobalIndexesForAmounts() const
{
    return state.keyOutputGlobalIndexesForAmounts;
}

const std::unordered_map<uint32_t, RawBlock> &
...
const std::unordered_map<uint64_t, std::vector<Crypto::Hash>> &
BlockchainReadResult::getBlockHashesByTimestamp() const
{
                                          ...
                                          const std::vector<bool> &resultStates)
{
    ...
    DB::deserializeValues (state.spentKeyImagesByBlock,
                           iter,
                           DB::BLOCK_INDEX_TO_KEY_IMAGE_PREFIX);
    DB::deserializeValues (state.blockIndexesBySpentKeyImages,
                           iter,
                           DB::KEY_IMAGE_TO_BLOCK_INDEX_PREFIX);
    DB::deserializeValues (state.cachedTransactions,
                           iter,
                           DB::TRANSACTION_HASH_TO_TRANSACTION_INFO_PREFIX);
    DB::deserializeValues (state.transactionHashesByBlocks,
                           iter,
                           DB::BLOCK_INDEX_TO_TX_HASHES_PREFIX);
    DB::deserializeValues (state.cachedBlocks,
                           iter,
                           DB::BLOCK_INDEX_TO_BLOCK_INFO_PREFIX);
    DB::deserializeValues (state.blockIndexesByBlockHashes,
                           iter,
                           DB::BLOCK_HASH_TO_BLOCK_INDEX_PREFIX);
    DB::deserializeValues (state.keyOutputGlobalIndexesCountForAmounts,
                           iter,
                           DB::KEY_OUTPUT_AMOUNT_PREFIX);
    DB::deserializeValues (state.keyOutputGlobalIndexesForAmounts,
                           iter,
                           DB::KEY_OUTPUT_AMOUNT_PREFIX);
    DB::deserializeValues (state.rawBlocks,
                           iter,
                           DB::BLOCK_INDEX_TO_RAW_BLOCK_PREFIX);
    DB::deserializeValues (state.closestTimestampBlockIndex,
                           iter,
                           DB::CLOSEST_TIMESTAMP_BLOCK_INDEX_PREFIX);
    DB::deserializeValues (state.keyOutputAmounts,
                           iter,
                           DB::KEY_OUTPUT_AMOUNTS_COUNT_PREFIX);
    DB::deserializeValues (state.transactionCountsByPaymentIds,
                           iter,
                           DB::PAYMENT_ID_TO_TX_HASH_PREFIX);
    DB::deserializeValues (state.transactionHashesByPaymentIds,
                           iter,
                           DB::PAYMENT_ID_TO_TX_HASH_PREFIX);
    DB::deserializeValues (state.blockHashesByTimestamp,
                           iter,
                           DB::TIMESTAMP_TO_BLOCKHASHES_PREFIX);
    DB::deserializeValues (state.keyOutputKeys,
                           iter,
                           DB::KEY_OUTPUT_KEY_PREFIX);

    DB::deserializeValue (state.lastBlockIndex,
                          iter,
                          DB::BLOCK_INDEX_TO_BLOCK_HASH_PREFIX);
    DB::deserializeValue (state.keyOutputAmountsCount,
                          iter,
                          DB::KEY_OUTPUT_AMOUNTS_COUNT_PREFIX);
    DB::deserializeValue (state.transactionsCount,
                          iter,
                          DB::TRANSACTION_HASH_TO_TRANSACTION_INFO_PREFIX);
...
BlockchainReadResult::BlockchainReadResult(BlockchainReadResult &&result)
