...
namespace CryptoNote {
    ...
    public:
        ...
        virtual bool getGlobalIndexesForRange(const uint64_t startHeight,
                                              ...
                                              std::unordered_map<Crypto::Hash,
                                                                 std::vector<uint64_t>> &indexes) const override;
    ...
    private:
        ...
        std::error_code validateSemantic(const Transaction &transaction,
                                         uint64_t &fee,
                                         uint32_t blockIndex);
        std::error_code validateTransaction(const CachedTransaction &transaction,
        ...
        std::error_code validateBlock(const CachedBlock &block,
                                      IBlockchainCache *cache,
        ...
        size_t pushBlockHashes(uint32_t startIndex,
                               uint32_t fullOffset,
        ...
        void fillQueryBlockFullInfo(uint32_t fullOffset,
                                    uint32_t currentIndex,
                                    size_t maxItemsCount,
                                    std::vector<BlockFullInfo> &entries) const;
        void fillQueryBlockShortInfo(uint32_t fullOffset,
                                     uint32_t currentIndex,
                                     size_t maxItemsCount,
                                     std::vector<BlockShortInfo> &entries) const;
        void fillQueryBlockDetails(uint32_t fullOffset,
                                   uint32_t currentIndex,
                                   size_t maxItemsCount,
                                   std::vector<BlockDetails> &entries) const;
        ...
        void mergeSegments(IBlockchainCache *acceptingSegment,
                           IBlockchainCache *segment);
        TransactionDetails getTransactionDetails(const Crypto::Hash &transactionHash,
                                                 IBlockchainCache *segment,
                                                 bool foundInPool) const;
        void notifyOnSuccess(error::AddBlockErrorCode opResult,
        ...
        */
        void actualizePoolTransactionsLite(const TransactionValidatorState &validatorState);
