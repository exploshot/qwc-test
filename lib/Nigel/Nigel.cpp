 ...
 */
inline std::shared_ptr<httplib::Client> getClient(const std::string daemonHost,
 ...
 */
Nigel::Nigel(const std::string daemonHost,
             const uint16_t daemonPort,
             const bool daemonSSL)
...
> Nigel::getWalletSyncData(const std::vector<Crypto::Hash> blockHashCheckpoints,
    ...
    const uint64_t requestedOuts) const
{
    ...
    /*!
     * The blockchain cache doesn't call it outs_count it calls it mixin
     */
