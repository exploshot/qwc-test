...
namespace CryptoNote {
    ...
    namespace {
        ...
        {
            return p2p.invokeNotifyToPeer (t_parametr::ID, LevinProtocol::encode (arg), context);
        }
    ...
    void CryptoNoteProtocolHandler::onConnectionClosed(CryptoNoteConnectionContext &context)
    {
        ...
        if (context.m_state != CryptoNoteConnectionContext::StateBeforeHandshake) {
            m_peersCount--;
    ...
    void CryptoNoteProtocolHandler::logConnections()
    {
        ...
        m_p2p->forEachConnection ([&](const CryptoNoteConnectionContext &cntxt, uint64_t peer_id)
                                    {
                                        ss
                                            << std::setw (25)
                                            << std::left
    ...
    bool CryptoNoteProtocolHandler::processPayloadSyncData(
        const CORE_SYNC_DATA &hshd,
        CryptoNoteConnectionContext &context,
        bool is_inital)
    {
        if (context.m_state == CryptoNoteConnectionContext::StateBeforeHandshake && !is_inital) {
            return true;
        }
        ...
        if (context.m_state == CryptoNoteConnectionContext::StateSynchronizing) {
            ...
            logger (logLevel, Logging::BRIGHT_GREEN)
                << ss0.str ();
            logger (logLevel, Logging::BRIGHT_GREEN)
                << ss1.str ();
            logger (logLevel, Logging::BRIGHT_GREEN)
    ...
    /*!
     * TODO: Changed std::bind -> lambda, for better debugging, remove it ASAP
     */
    ...
    int CryptoNoteProtocolHandler::handleNotifyNewTransactions(
...
*/
    ...
    bool
    CryptoNoteProtocolHandler::requestMissingObjects(CryptoNoteConnectionContext &context, bool check_having_blocks)
    {
    ...
    bool CryptoNoteProtocolHandler::onConnectionSynchronized()
    {
        ...
        if (m_synchronized.compare_exchange_strong (val_expected, true)) {
            ...
            logger (INFO, BRIGHT_MAGENTA)
                ...
                << ENDL;
            logger (INFO, WHITE)
                << " Always exit " + WalletConfig::daemonName + " and " + WalletConfig::walletName
            ...
            logger (INFO, WHITE)
                ...
                << ENDL;
            logger (INFO, WHITE)
                << " Use the \"backup\" command in " + WalletConfig::walletName
                                                            ...
                                                            CryptoNoteConnectionContext &context)
    {
        logger (Logging::TRACE)
            << context
            << "NOTIFY_RESPONSE_CHAIN_ENTRY: m_block_ids.size()="
        ...
        if (!arg.m_block_ids.size ()) {
            logger (Logging::ERROR)
                << context
                << "sent empty m_block_ids, dropping connection";
            context.m_state = CryptoNoteConnectionContext::StateShutdown;
            return 1;
        }
        ...
        if (!m_core.hasBlock (arg.m_block_ids.front ())) {
            logger (Logging::ERROR)
                << context
                << "sent m_block_ids starting from unknown id: "
    ...
    int CryptoNoteProtocolHandler::handleNotifyNewLiteBlock(int command, NOTIFY_NEW_LITE_BLOCK::request &arg,
                                                            CryptoNoteConnectionContext &context)
    {
        logger (Logging::TRACE)
            << context
    ...
    int CryptoNoteProtocolHandler::handleNotifyMissingTxs(int command, NOTIFY_MISSING_TXS::request &arg,
                                                          CryptoNoteConnectionContext &context)
    {
        logger (Logging::TRACE)
            << context
    ...
    void CryptoNoteProtocolHandler::relayBlock(NOTIFY_NEW_BLOCK::request &arg)
    {
        ...
        m_p2p
            ->forEachConnection ([this,
                                    &liteBlockConnections,
                                    &normalBlockConnections](const CryptoNoteConnectionContext &ctx,
                                                             uint64_t peerId)
                                   {
    ...
    void CryptoNoteProtocolHandler::recalculateMaxObservedHeight(const CryptoNoteConnectionContext &context)
    {
        ...
        m_p2p->forEachConnection ([&peerHeight, &context](const CryptoNoteConnectionContext &ctx, uint64_t peerId)
                                    {
